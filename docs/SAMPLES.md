...
samples/
  ...
  arena_container_helpers.h   ArenaHelpers::AppendMapEntry -- map_properties helper for the proto/native/binary paths
...
All four formats carry the **full container set** -- scalar arrays, a nested struct, an array-of-structs inventory, and a map -- and produce byte-identical `PropertyContainer`s (equal per-entity `content_hash`). Scalar arrays, nested structs and struct arrays flow through each loader automatically; the **map** (`AmmoByWeapon`) is the one asymmetry: the FlatBuffers loader recognises `containerType: Map` and builds `map_properties` for you, while the Protobuf, native and binary bindings call `ArenaHelpers::AppendMapEntry` (from `arena_container_helpers.h`) to populate it with the same key convention.
...
| Populate a `map_properties` field by hand (proto / native / binary paths) | `arena_container_helpers.h` -- `ArenaHelpers::AppendMapEntry` |
