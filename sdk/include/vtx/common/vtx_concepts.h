...
namespace VTX {
    ...
    concept VtxScalarType =
        ...
        std::same_as<T, VTX::Vector> || std::same_as<T, VTX::Quat> || std::same_as<T, VTX::Transform> ||
        std::same_as<T, VTX::FloatRange> || std::same_as<T, VTX::PropertyContainer> ||
        std::same_as<T, class EntityView>;
    ...
    template <typename T>
    concept IVtxWriterPolicy =
        requires(const typename T::HeaderType& header_config, const typename T::SchemaType& schema,
                 std::vector<std::unique_ptr<typename T::FrameType>>& frames, int32_t chunk_index, bool is_compressed,
                 const std::vector<ChunkIndexData>& seek_table, const SessionFooter& footer_data) {
    ...
    template <typename T>
    concept IVtxReaderPolicy =
        requires(std::string buffer, const typename T::HeaderType& header, const typename T::FooterType& footer,
                 std::vector<VTX::ChunkIndexEntry> index_table, VTX::GameTime::VTXGameTimes& game_times,
                 int32_t chunk_idx, std::stop_token stop_token, std::vector<VTX::Frame>& out_native_frames,
            ...
            {
                T::ProcessChunkData(chunk_idx, buffer, stop_token, out_native_frames, out_decompressed_blob,
                                    out_raw_frames_spans)
                } -> std::same_as<void>;
        };
