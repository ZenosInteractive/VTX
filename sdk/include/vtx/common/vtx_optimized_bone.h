...
namespace VTX 
{
    ...
    struct alignas(16) OptimizedBoneData
    {
        ...
        void Pack(const Transform& SourceTransform, const BoneCompressionSettings& Settings)
        {
            ...
            // Normalize Quat
            float invLen = 1.0f / std::sqrt(NormalizedQuat.x * NormalizedQuat.x + 
            ...
            auto PackComponent = [&](int offset) -> uint32_t {
                ...
                return std::clamp(static_cast<uint32_t>(std::round((comp + 0.707107f) * RotationPackFactor)), 0u, 8191u);
            };
            ...
            if (Settings.use_high_precision_position_no_scale)
            {
                ...
                auto PackPos = [&](double locAxis) -> uint32_t {
                    return std::clamp(static_cast<uint32_t>(std::round((static_cast<float>(locAxis) - Settings.position_minimum) * PosScaleFactor)), 0u, 2097151u);
                };
            ...
            else
            {
                ...
                auto PackPos = [&](double locAxis) -> uint32_t {
                    return std::clamp(static_cast<uint32_t>(std::round((static_cast<float>(locAxis) - Settings.position_minimum) * PosScaleFactor)), 0u, 65535u);
                };
                ...
                auto PackScale = [&](double scaleAxis) -> uint32_t {
                    return std::clamp(static_cast<uint32_t>(std::round((static_cast<float>(scaleAxis) - Settings.scale_minimum) * ScalePackFactor)), 0u, 255u);
                };
        ...
        void Unpack(const BoneCompressionSettings& Settings, Transform& OutTransform) const
        {
            ...
            OutTransform.rotation = { 
                QuatComponents[0], QuatComponents[1], QuatComponents[2], QuatComponents[3] 
            };
            ...
            if (Settings.use_high_precision_position_no_scale)
            {
                ...
                OutPosition.x = static_cast<double>(static_cast<float>(RawPosX) * PosUnpackFactor + Settings.position_minimum);
                OutPosition.y = static_cast<double>(static_cast<float>(RawPosY) * PosUnpackFactor + Settings.position_minimum);
                OutPosition.z = static_cast<double>(static_cast<float>(RawPosZ) * PosUnpackFactor + Settings.position_minimum);
            }
            else
            {
                ...
                OutScale.x = static_cast<double>(static_cast<float>(RawScaleX) * ScaleUnpackFactor + Settings.scale_minimum);
                OutScale.y = static_cast<double>(static_cast<float>(RawScaleY) * ScaleUnpackFactor + Settings.scale_minimum);
                OutScale.z = static_cast<double>(static_cast<float>(RawScaleZ) * ScaleUnpackFactor + Settings.scale_minimum);
            }
