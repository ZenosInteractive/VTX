...
namespace VTX
{
    ...
    struct FlatArray {
        ...
        {
            if (SubIndex > offsets.size()) return false;
        ...
        bool EraseRange(size_t SubIndex, size_t First, size_t Last)
        {
            ...
            if (First > Last || Last > Sz) return false;
    ...
    struct Bucket {
        ...
        std::vector<EntityRange> type_ranges; //< Ranges to know were a type starts and ends, assumes entites are ordered by type
    ...
    struct Frame {
        ...
        Bucket& GetBucket(int32_t bucket_index)
        {
    ...
    namespace GameTime
    {
        enum class EGameTimeType : uint8_t {
            None,
            OnlyGameTime,
            OnlyCreatedUtc,
            Both
        ...
        struct VTXGameTimes
        {
            ...
            {
                game_time_.insert(game_time_.end(), NewChunkGameTime.game_time_.begin(), NewChunkGameTime.game_time_.end());
                created_utc_.insert(created_utc_.end(), NewChunkGameTime.created_utc_.begin(), NewChunkGameTime.created_utc_.end());
                receive_utc_.insert(receive_utc_.end(), NewChunkGameTime.receive_utc_.begin(), NewChunkGameTime.receive_utc_.end());
                game_times_sorted_as_ticks_.insert(game_times_sorted_as_ticks_.end(), NewChunkGameTime.game_times_sorted_as_ticks_.begin(), NewChunkGameTime.game_times_sorted_as_ticks_.end());
                sorted_game_time_indexes_.insert(sorted_game_time_indexes_.end(), NewChunkGameTime.sorted_game_time_indexes_.begin(), NewChunkGameTime.sorted_game_time_indexes_.end());
                timeline_gaps_.insert(timeline_gaps_.end(), NewChunkGameTime.timeline_gaps_.begin(), NewChunkGameTime.timeline_gaps_.end());
                game_segments_.insert(game_segments_.end(), NewChunkGameTime.game_segments_.begin(), NewChunkGameTime.game_segments_.end());
            }
            ...
            void ManuallyMarkGameSegmentStart(const int32 FrameNumber)
            {
                game_segments_.push_back(FrameNumber);
            }

            void ManuallyMarkGameSegmentStart()
            {
            ...
            bool IsEmpty() const
            {
                return game_time_.empty() && created_utc_.empty() && receive_utc_.empty();
            }
            ...
            void DebugPrintGameTimes()
            {
                ...
                {
                    VTX_DEBUG("Frame {}: game_time_ = {}, CreatedUTC = {}", i, TicksToString(game_time_[i]), TicksToString(created_utc_[i]));
                }
            }

            void UpdateChunkStartIndex()
            {
                chunk_start_index_ = GetFrameNumber();
            }
            ...
            std::vector<int32> GetLastChunkSortedGameTimeIndexes() const { return GetLastChunk(sorted_game_time_indexes_); }
            std::vector<int64> GetLastChunkGameTimesSortedAsTicks() const { return GetLastChunk(game_times_sorted_as_ticks_); }
            ...
            void SetGameTimesSortedAsTicks(std::vector<int64_t> ticks) { game_times_sorted_as_ticks_ = std::move(ticks); }
            void SetSortedGameTimeIndexes(std::vector<int32_t> indexes) { sorted_game_time_indexes_ = std::move(indexes); }
        ...
        private:
            ...
            bool Resolve(const int32 FrameAmountSoFar)
            {
                ...
                if (game_time_.size() == created_utc_.size() && game_time_.size() == static_cast<size_t>(FrameAmountSoFar))
                {
                    if (!game_time_.empty())
                    {
                        SortGameTime();
            ...
            void FakeGameTimeFromUtc()
            {
                game_time_.push_back(created_utc_.back() - created_utc_[0]);
            }
            ...
            void SortGameTime()
            {
                ...
                auto it = std::lower_bound(game_times_sorted_as_ticks_.begin(), game_times_sorted_as_ticks_.end(), NewTime);
            ...
            void DetectGameSegment()
            {
                ...
                for(int32 val : game_segments_) { if(val == currentFrame) { bContains = true; break; } }
    ...
    struct SessionFooter {
        SessionFooter() : total_frames(0), duration_seconds(0)
        {
        }
    ...
    struct ChunkIndexData {
        ChunkIndexData() : chunk_index(0), file_offset(0), chunk_size_bytes(0), start_frame(0), end_frame(0)
        {
        }
