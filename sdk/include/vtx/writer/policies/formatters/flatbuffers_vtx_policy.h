...
namespace VTX {
    ...
    struct FlatBuffersVtxPolicy {
        ...
        static std::string SerializeChunk(const std::vector<std::unique_ptr<FrameType>>& frames, int32_t chunk_idx,
                                          bool is_compressed);
        static std::string SerializeFooter(const std::vector<ChunkIndexData>& seek_table,
