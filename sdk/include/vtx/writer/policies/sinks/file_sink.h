...
namespace VTX
{
    ...
    class ChunkedFileSink
    {
        ...
        using SerializerPolicy = Policy;
            ...
            explicit ChunkedFileSink(Config config) : config_(std::move(config)) {
                file_.open(config_.filename, std::ios::binary | std::ios::out | std::ios::trunc);
                if (!file_.is_open()) throw std::runtime_error("VTX: Could not open " + config_.filename);
            }
        ...
        private:
            void WriteBlob(const std::string& data) {
