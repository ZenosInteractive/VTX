...
inline void vtx_close_socket(VtxSocketHandle s) { ::closesocket(s); }
...
inline void vtx_close_socket(VtxSocketHandle s) { ::close(s); }
...
namespace VTX {
    ...
    public:
        ...
        explicit ChunkedNetworkSink(Config config) : config_(std::move(config)) {
        ...
        void SaveChunk(std::vector<std::unique_ptr<FrameType>>& frames,
                       const std::vector<int64_t>& /*created_utc*/,
    ...
    private:
        ...
        std::string CompressIfBeneficial(std::string payload) {
            ...
            size_t      compressed_size = ZSTD_compress(compressed.data(), max_size,
