...
namespace VTX {
    ...
    public:
        explicit SchemaDynamicLoader(const SchemaRegistry& registry, const SchemaSanitizerRegistry* sanitizer = nullptr,
        ...
        void Load(const AdapterType& adapter, const std::string& structName, PropertyContainer& outContainer) {
            if (debug_mode_)
                std::cout << "[Loader] Loading struct: '" << structName << "'" << std::endl;

            if (!registry_) {
                if (debug_mode_)
                    std::cerr << "[Loader][Error] Schema is null: " << std::endl;
                return;
            }
            ...
            if (!schema) {
                if (debug_mode_)
                    std::cerr << "[Loader][Error] Schema not found: " << structName << std::endl;
                return;
            }
        ...
        void LoadSingleField(const AdapterType& node, const SchemaField& field, PropertyContainer& out) {
            ...
            try {
                ...
                case FieldType::Int32:
                    out.int32_properties[idx] = node.template GetValue<int32_t>();
                    break;
                case FieldType::Int64:
                    out.int64_properties[idx] = node.template GetValue<int64_t>();
                    break;
                case FieldType::Float:
                    out.float_properties[idx] = node.template GetValue<float>();
                    break;
                case FieldType::Double:
                    out.double_properties[idx] = node.template GetValue<double>();
                    break;
                case FieldType::Bool:
                    out.bool_properties[idx] = node.template GetValue<bool>();
                    break;
                case FieldType::String:
                    out.string_properties[idx] = node.template GetValue<std::string>();
                    break;

                case FieldType::Vector: {
                    out.vector_properties[idx] = {node.GetChild("x").template GetValue<double>(),
                ...
                case FieldType::Quat: {
                    out.quat_properties[idx] = {
                        node.GetChild("x").template GetValue<float>(), node.GetChild("y").template GetValue<float>(),
                        node.GetChild("z").template GetValue<float>(), node.GetChild("w").template GetValue<float>()};
                    break;
                }
                case FieldType::FloatRange: {
                    out.range_properties[idx] = {node.GetChild("min").template GetValue<float>(),
                ...
                case FieldType::Transform: {
                    ...
                    t.translation = {loc.GetChild("x").template GetValue<double>(),
                                     loc.GetChild("y").template GetValue<double>(),
                                     loc.GetChild("z").template GetValue<double>()};

                    auto rot = node.GetChild("rotation");
                    t.rotation = {
                        rot.GetChild("x").template GetValue<float>(), rot.GetChild("y").template GetValue<float>(),
                        rot.GetChild("z").template GetValue<float>(), rot.GetChild("w").template GetValue<float>()};

                    auto scl = node.GetChild("scale");
                    t.scale = {scl.GetChild("x").template GetValue<double>(),
                               scl.GetChild("y").template GetValue<double>(),
                               scl.GetChild("z").template GetValue<double>()};
                ...
                if (debug_mode_)
                    std::cerr << "[Loader] Error loading field '" << field.name << "': " << e.what() << std::endl;
            }
        }
