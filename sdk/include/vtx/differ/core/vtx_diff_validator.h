...
namespace VtxDiff {
    ...
    class DiffValidator {
    public:
        static bool ValidatePatch(const TNodeView& NodeA, const TNodeView& NodeB, const PatchIndex& Patch,
        ...
        static void PrintPatch(const PatchIndex& Patch, size_t max_ops_to_print = 50) {
            ...
            for (size_t i = 0; i < limit; ++i) {
                ...
                case DiffOperation::Replace:
                    ...
                    break;
                case DiffOperation::ReplaceRange:
                    op_name = "[REPLACE RANGE]";
                    break;
                }

                std::cout << std::left << std::setw(17) << op_name << " | Field: " << std::setw(22)
                          << TypeToFieldName(op.ContainerType);
    ...
    private:
        static bool AreBytesEqual(std::span<const std::byte> BytesA, std::span<const std::byte> BytesB,
                                  const DiffOptions& Opt) {
