...
namespace VtxDiff {
    ...
    inline bool ReadOffsetU32(std::span<const std::byte> Raw, size_t& Out) {
        if (Raw.size() != sizeof(uint32_t))
            return false;
        uint32_t v = 0;
        std::memcpy(&v, Raw.data(), sizeof(uint32_t));
        Out = (size_t)v;
        return true;
    }
    ...
    private:
        ...
        static int32_t HashToPathKey(uint64_t Hash) { return static_cast<int32_t>(Hash & 0x7FFFFFFF); }
    ...
    public:
        ...
        PatchIndex ComputeDiff(const TNodeView& NodeA, const TNodeView& NodeB, const DiffOptions& Opt) const {
            ...
            if (!NodeA.IsValid() && !NodeB.IsValid())
                return Out;
            if (!NodeA.IsValid() && NodeB.IsValid()) {
                Out.operations.push_back({DiffOperation::Add, EVTXContainerType::Unknown, root});
                return Out;
            }
            if (NodeA.IsValid() && !NodeB.IsValid()) {
                Out.operations.push_back({DiffOperation::Remove, EVTXContainerType::Unknown, root});
                return Out;
            }
        ...
        PatchIndex ComputeEntityDiff(const TNodeView& EntityA, const TNodeView& EntityB, const DiffOptions& Opt,
                                     DiffIndexPath BasePath = {}) const {
            PatchIndex Out;
            Out.operations.reserve(512);
            if (!EntityA.IsValid() && !EntityB.IsValid())
                return Out;
            if (!EntityA.IsValid() && EntityB.IsValid()) {
                Out.operations.push_back({DiffOperation::Add, EVTXContainerType::Unknown, BasePath});
                return Out;
            }
            if (EntityA.IsValid() && !EntityB.IsValid()) {
                Out.operations.push_back({DiffOperation::Remove, EVTXContainerType::Unknown, BasePath});
                return Out;
            }
    ...
    private:
        void Diff(const TNodeView& NodeA, const TNodeView& NodeB, const DiffOptions& Opt, DiffIndexPath& Path,
                  PatchIndex& Out) const {
            ...
            if (!validA && !validB)
                return;
            if (!validA && validB) {
                Out.operations.push_back({DiffOperation::Add, EVTXContainerType::Unknown, Path});
                return;
            }
            if (validA && !validB) {
                Out.operations.push_back({DiffOperation::Remove, EVTXContainerType::Unknown, Path});
                return;
            }
            ...
            for (const auto& f : FieldsA) {
                if (f.is_actors_field || f.type == EVTXContainerType::Unknown)
                    continue;
            ...
            for (uint32_t t = 0; t <= maxTypeSeen; ++t) {
                ...
                if (!Fda && Fdb) {
                    ...
                    case EVTXContainerType::RangeProperties:
                        DiffScalarArray(NodeA, NodeB, *Fda, *Fdb, Opt, Out, Path);
                        break;
                    case EVTXContainerType::AnyStructProperties:
                        DiffStructArray(NodeA, NodeB, *Fda, *Fdb, Opt, Out, Path);
                        break;
                    case EVTXContainerType::MapProperties:
                        DiffMapContainers(NodeA, NodeB, *Fda, *Fdb, Opt, Out, Path);
                        break;
                    default:
                        break;
                    }
        ...
        void DiffActorsArray(const TNodeView& NodeA, const TNodeView& NodeB, const FieldDesc& Fda, const FieldDesc& Fdb,
                             const DiffOptions& Opt, PatchIndex& Out, DiffIndexPath& Path) const {
            ...
            std::sort(ScratchActorsB.begin(), ScratchActorsB.end(),
                      [](const ActorEntry& a, const ActorEntry& b) { return a.Hash < b.Hash; });
            ...
            for (size_t i = 0; i < CountA; ++i) {
                ...
                if (i < CountB) {
                    ...
                    std::string_view IdViewB_Direct(reinterpret_cast<const char*>(spanIdB_Direct.data()),
                                                    spanIdB_Direct.size());
                ...
                if (foundIndexB == static_cast<size_t>(-1)) {
                    ...
                    Out.operations.push_back(
                        {DiffOperation::Remove, EVTXContainerType::AnyStructProperties, Path, IdStr});
                    Path.pop_back();
                    continue;
                }
        ...
        void DiffScalarArray(const TNodeView& A, const TNodeView& B, const FieldDesc& Fda, const FieldDesc& Fdb,
                             const DiffOptions& Opt, PatchIndex& Out, const DiffIndexPath& Path) const {
            ...
            if (!full_bytes_a.empty() && full_bytes_a.size() == full_bytes_b.size()) {
                if (std::memcmp(full_bytes_a.data(), full_bytes_b.data(), full_bytes_a.size()) == 0)
                    return;
            }
                    ...
                    for (size_t i = 0; i < size; ++i) {
                        ...
                        if (Opt.compare_floats_with_epsilon) {
                            if (stride_a == sizeof(float)) {
                                ...
                                bDifferent =
                                    (std::memcmp(ptr_a + (i * stride_a), ptr_b + (i * stride_b), stride_a) != 0);
                            }
                        } else {
                            auto span_a = std::span<const std::byte>(
                                reinterpret_cast<const std::byte*>(ptr_a + (i * stride_a)), stride_a);
                            auto span_b = std::span<const std::byte>(
                                reinterpret_cast<const std::byte*>(ptr_b + (i * stride_b)), stride_a);
                            bDifferent = !FastSpanEqual(span_a, span_b);
                        }

                        if (bDifferent) {
                            if (current_range_start == -1)
                                current_range_start = static_cast<int32_t>(i);
                            ++current_range_count;
                        } else {
                            FlushRange();
                        }
            ...
            for (size_t i = 0; i < size; ++i) {
                ...
                if (bytes_a.empty() && bytes_b.empty()) {
                    FlushRange();
                    continue;
                }
                ...
                if (Opt.compare_floats_with_epsilon && bytes_a.size() == bytes_b.size()) {
                    if (bytes_a.size() == sizeof(float)) {
                        float fa, fb;
                        std::memcpy(&fa, bytes_a.data(), sizeof(float));
                        std::memcpy(&fb, bytes_b.data(), sizeof(float));
                        bDifferent = std::abs(fa - fb) > Opt.float_epsilon;
                    } else if (bytes_a.size() == sizeof(double)) {
                        double da, db;
                        std::memcpy(&da, bytes_a.data(), sizeof(double));
                        std::memcpy(&db, bytes_b.data(), sizeof(double));
                        bDifferent = std::abs(da - db) > static_cast<double>(Opt.float_epsilon);
                    } else {
        ...
        void DiffStructArray(const TNodeView& A, const TNodeView& B, const FieldDesc& Fda, const FieldDesc& Fdb,
                             const DiffOptions& Opt, PatchIndex& Out, DiffIndexPath& Path) const {
            auto full_bytes_a = A.GetFieldBytes(Fda);
            auto full_bytes_b = B.GetFieldBytes(Fdb);
        ...
        void DiffFlatRepeatedByOffsets(const TNodeView& A, const TNodeView& B, const FieldDesc& FlatFda,
                                       const FieldDesc& FlatFdb, const FieldDesc& OffFda, const FieldDesc& OffFdb,
                                       const DiffOptions& Opt, PatchIndex& Out, DiffIndexPath& Path) const {
            ...
            for (size_t i = 0; i < N; ++i) {
                ...
                if (!bValidOffsets) {
                    Out.operations.push_back(
                        {DiffOperation::Replace, FlatFda.type, Path.Append(static_cast<int32_t>(i))});
                    continue;
                }
                ...
                if (i + 1 < OffCountA) {
                    size_t Next = 0;
                    if (ReadOffsetU32(A.GetArrayElementBytes(OffFda, i + 1), Next))
                        EndA = Next;
                }
                if (i + 1 < OffCountB) {
                    size_t Next = 0;
                    if (ReadOffsetU32(B.GetArrayElementBytes(OffFdb, i + 1), Next))
                        EndB = Next;
                }
                ...
                if (FlatFda.type == EVTXContainerType::AnyStructArrays) {
                    for (size_t j = 0; j < LenA; ++j) {
                        ...
                        if (!NodeElemA.IsValid() || !NodeElemB.IsValid()) {
                            bSubArrayChanged = true;
                            break;
                        }
                    ...
                    for (size_t j = 0; j < LenA; ++j) {
                        ...
                        if (!AreScalarsEqual(FlatFda.type, EA, EB, Opt)) {
                            bSubArrayChanged = true;
                            break;
                        }
                    }
            ...
            for (size_t i = N; i < OffCountA; ++i) {
                Out.operations.push_back({DiffOperation::Remove, FlatFda.type, Path.Append(static_cast<int32_t>(i))});
            }
            for (size_t i = N; i < OffCountB; ++i) {
                Out.operations.push_back({DiffOperation::Replace, FlatFda.type, Path.Append(static_cast<int32_t>(i))});
            }
        }

        void DiffEncapsulatedArray(const TNodeView& ParentA, const TNodeView& ParentB, const FieldDesc& WrapperFda,
                                   const FieldDesc& WrapperFdb, const DiffOptions& Opt, PatchIndex& Out,
                                   DiffIndexPath& Path) const {
            auto wrap_a = ParentA.GetNestedStruct(WrapperFda);
            auto wrap_b = ParentB.GetNestedStruct(WrapperFdb);

            if (!wrap_a.IsValid() && !wrap_b.IsValid())
                return;
            if (!wrap_a.IsValid() && wrap_b.IsValid()) {
                Out.operations.push_back({DiffOperation::Add, WrapperFda.type, Path});
                return;
            }
            if (wrap_a.IsValid() && !wrap_b.IsValid()) {
                Out.operations.push_back({DiffOperation::Remove, WrapperFda.type, Path});
                return;
            }

            FieldDesc field_desc;
            field_desc.name = "offsets";
            FieldDesc data_desc;
            data_desc.name = "data";
            ...
            for (size_t i = 0; i < size; ++i) {
                ...
                if (!b_valid) {
                    Out.operations.push_back(
                        {DiffOperation::Replace, WrapperFda.type, Path.Append(static_cast<int32_t>(i))});
                    continue;
                }
                ...
                if (i + 1 < off_count_a) {
                    size_t Next = 0;
                    if (ReadOffsetAny(wrap_a.GetArrayElementBytes(field_desc, i + 1), Next))
                        end_a = Next;
                }
                if (i + 1 < off_count_b) {
                    size_t Next = 0;
                    if (ReadOffsetAny(wrap_b.GetArrayElementBytes(field_desc, i + 1), Next))
                        end_b = Next;
                }
                ...
                if (b_is_recursive) {
                    ...
                    if (!block_a.empty() && block_a.size() == block_b.size()) {
                        if (std::memcmp(block_a.data(), block_b.data(), block_a.size()) == 0)
                            continue;
                    }
                    ...
                    for (size_t j = 0; j < len_a; ++j) {
                        ...
                        if (!AreScalarsEqual(WrapperFda.type, bytes_a, bytes_b, Opt)) {
                            b_changed = true;
                            break;
                        }
                    }
            ...
            for (size_t i = size; i < off_count_a; ++i) {
                Out.operations.push_back(
                    {DiffOperation::Remove, WrapperFda.type, Path.Append(static_cast<int32_t>(i))});
            }
            for (size_t i = size; i < off_count_b; ++i) {
                Out.operations.push_back(
                    {DiffOperation::Replace, WrapperFda.type, Path.Append(static_cast<int32_t>(i))});
            }
        }

        void DiffMapContainers(const TNodeView& A, const TNodeView& B, const FieldDesc& Fda, const FieldDesc& Fdb,
                               const DiffOptions& Opt, PatchIndex& Out, DiffIndexPath& Path) const {
            ...
            std::sort(ScratchMapA.begin(), ScratchMapA.end(),
                      [](const auto& a, const auto& b) { return a.Hash < b.Hash; });
            std::sort(ScratchMapB.begin(), ScratchMapB.end(),
                      [](const auto& a, const auto& b) { return a.Hash < b.Hash; });

            size_t idx_a = 0, idx_b = 0;
            while (idx_a < ScratchMapA.size() || idx_b < ScratchMapB.size()) {
                if (idx_a < ScratchMapA.size() &&
                    (idx_b == ScratchMapB.size() || ScratchMapA[idx_a].Hash < ScratchMapB[idx_b].Hash)) {
                    ...
                    Path.pop_back();
                    idx_a++;
                } else if (idx_b < ScratchMapB.size() &&
                           (idx_a == ScratchMapA.size() || ScratchMapB[idx_b].Hash < ScratchMapA[idx_a].Hash)) {
