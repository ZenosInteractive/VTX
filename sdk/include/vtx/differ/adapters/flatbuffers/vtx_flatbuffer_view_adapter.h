...
namespace VtxDiff::Flatbuffers {
    ...
    public:
        ...
        FlatbufferViewAdapter(const FbSchemaCache* Cache, const reflection::Object* Object, const flatbuffers::Table* TablePtr);
        ...
        static std::optional<FlatbufferViewAdapter> CreateRoot(
            const FbSchemaCache* Cache,
            const char* RootObjectName, const uint8_t* Buffer, size_t BufferSize);

    private:
        const reflection::Field* FindField(std::string_view FieldName) const;
        const uint8_t* GetVectorData(const flatbuffers::Table* Tbl, const reflection::Field* F, size_t& Count, size_t& ElemSize) const;
    ...
    static_assert(CBinaryNodeView<FlatbufferViewAdapter>, "FlatbufferViewAdapter does not follow CBinaryNodeView concept!");
