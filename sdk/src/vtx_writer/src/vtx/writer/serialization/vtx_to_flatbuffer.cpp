...
namespace VTX {
    namespace Serialization {
        ...
        auto CreateScalarArray(flatbuffers::FlatBufferBuilder& builder, const std::vector<T>& data,
                               const std::vector<uint32_t>& offsets, CreateFunc func) {
            if (data.empty())
                return decltype(func(builder, 0, 0))(0);
        ...
        auto CreateStructArray(flatbuffers::FlatBufferBuilder& builder, std::vector<NativeT>& data,
                               const std::vector<uint32_t>& offsets, CreateFunc func) {
            if (data.empty())
                return decltype(func(builder, 0, 0))(0);
        ...
        static auto CreateStringArray(flatbuffers::FlatBufferBuilder& builder, const std::vector<std::string>& data,
                                      const std::vector<uint32_t>& offsets, CreateFunc func) {
            if (data.empty())
                return decltype(func(builder, 0, 0))(0);
            ...
            for (const auto& s : data)
                str_offsets.push_back(builder.CreateString(s));
        ...
        template <typename CreateFunc>
        static auto CreateBoolFromBitArray(flatbuffers::FlatBufferBuilder& builder, const std::vector<bool>& data,
                                           const std::vector<uint32_t>& offsets, CreateFunc func) {
            if (data.empty())
                return decltype(func(builder, 0, 0))(0);
...
flatbuffers::Offset<fbsvtx::MapContainer> VTX::Serialization::ToFlat(flatbuffers::FlatBufferBuilder& builder,
    ...
    for (const auto& s : src.keys)
        k.push_back(builder.CreateString(s));

    std::vector<flatbuffers::Offset<fbsvtx::PropertyContainer>> v;
    v.reserve(src.values.size());
    for (auto& p : src.values)
        v.push_back(ToFlat(builder, p));

    return fbsvtx::CreateMapContainer(builder, builder.CreateVector(k), builder.CreateVector(v));
}

flatbuffers::Offset<fbsvtx::PropertyContainer> VTX::Serialization::ToFlat(flatbuffers::FlatBufferBuilder& builder,
    ...
    if (!src.string_properties.empty()) {
        ...
        for (const auto& s : src.string_properties)
    ...
    if (!src.vector_properties.empty()) {
        std::vector<fbsvtx::Vector> temp;
        temp.reserve(src.vector_properties.size());
        for (auto& v : src.vector_properties)
            temp.push_back(ToFlat(v));
    ...
    if (!src.quat_properties.empty()) {
        std::vector<fbsvtx::Quat> temp;
        temp.reserve(src.quat_properties.size());
        for (auto& v : src.quat_properties)
            temp.push_back(ToFlat(v));
    ...
    if (!src.transform_properties.empty()) {
        std::vector<fbsvtx::Transform> temp;
        temp.reserve(src.transform_properties.size());
        for (auto& v : src.transform_properties)
            temp.push_back(ToFlat(v));
    ...
    if (!src.range_properties.empty()) {
        std::vector<fbsvtx::FloatRange> temp;
        temp.reserve(src.range_properties.size());
        for (auto& v : src.range_properties)
            temp.push_back(ToFlat(v));
    ...
    // Struct Arrays
    auto flatVec = CreateStructArray<VTX::Vector, fbsvtx::Vector>(
        builder, src.vector_arrays.data, src.vector_arrays.offsets,
        [](auto& b, auto d, auto o) { return fbsvtx::CreateFlatVectorArray(b, d, o); });

    auto flatQuat = CreateStructArray<VTX::Quat, fbsvtx::Quat>(
        builder, src.quat_arrays.data, src.quat_arrays.offsets,
        [](auto& b, auto d, auto o) { return fbsvtx::CreateFlatQuatArray(b, d, o); });

    auto flatTrans = CreateStructArray<VTX::Transform, fbsvtx::Transform>(
        builder, src.transform_arrays.data, src.transform_arrays.offsets,
        [](auto& b, auto d, auto o) { return fbsvtx::CreateFlatTransformArray(b, d, o); });

    auto flatRange = CreateStructArray<VTX::FloatRange, fbsvtx::FloatRange>(
        builder, src.range_arrays.data, src.range_arrays.offsets,
    ...
    if (!src.any_struct_properties.empty()) {
        ...
        for (auto& child : src.any_struct_properties)
            offs.push_back(ToFlat(builder, child));
    ...
    if (!src.any_struct_arrays.data.empty()) {
        ...
        for (auto& item : src.any_struct_arrays.data)
            d.push_back(ToFlat(builder, item));

        anyStructArrOff = fbsvtx::CreateFlatPropertyContainerArray(builder, builder.CreateVector(d),
                                                                   builder.CreateVector(src.any_struct_arrays.offsets));
    }
    ...
    if (!src.map_properties.empty()) {
        ...
        for (auto& item : src.map_properties)
            m.push_back(ToFlat(builder, item));
    ...
    if (!src.map_arrays.data.empty()) {
        ...
        for (auto& item : src.map_arrays.data)
            d.push_back(ToFlat(builder, item));
        mapArrOff =
            fbsvtx::CreateFlatMapArray(builder, builder.CreateVector(d), builder.CreateVector(src.map_arrays.offsets));
    }
    ...
    if (!boolOff.IsNull())
        containerBuilder.add_bool_properties(boolOff);
    if (!int32Off.IsNull())
        containerBuilder.add_int32_properties(int32Off);
    if (!int64Off.IsNull())
        containerBuilder.add_int64_properties(int64Off);
    if (!floatOff.IsNull())
        containerBuilder.add_float_properties(floatOff);
    if (!doubleOff.IsNull())
        containerBuilder.add_double_properties(doubleOff);
    if (!stringOff.IsNull())
        containerBuilder.add_string_properties(stringOff);

    if (!vecPropOff.IsNull())
        containerBuilder.add_vector_properties(vecPropOff);
    if (!quatPropOff.IsNull())
        containerBuilder.add_quat_properties(quatPropOff);
    if (!transPropOff.IsNull())
        containerBuilder.add_transform_properties(transPropOff);
    if (!rangePropOff.IsNull())
        containerBuilder.add_range_properties(rangePropOff);

    if (!flatBytes.IsNull())
        containerBuilder.add_byte_array_properties(flatBytes);
    if (!flatInt32.IsNull())
        containerBuilder.add_int32_arrays(flatInt32);
    if (!flatInt64.IsNull())
        containerBuilder.add_int64_arrays(flatInt64);
    if (!flatFloat.IsNull())
        containerBuilder.add_float_arrays(flatFloat);
    if (!flatDouble.IsNull())
        containerBuilder.add_double_arrays(flatDouble);
    if (!flatString.IsNull())
        containerBuilder.add_string_arrays(flatString);
    if (!flatBool.IsNull())
        containerBuilder.add_bool_arrays(flatBool);

    if (!flatVec.IsNull())
        containerBuilder.add_vector_arrays(flatVec);
    if (!flatQuat.IsNull())
        containerBuilder.add_quat_arrays(flatQuat);
    if (!flatTrans.IsNull())
        containerBuilder.add_transform_arrays(flatTrans);
    if (!flatRange.IsNull())
        containerBuilder.add_range_arrays(flatRange);

    if (!anyStructOff.IsNull())
        containerBuilder.add_any_struct_properties(anyStructOff);
    if (!anyStructArrOff.IsNull())
        containerBuilder.add_any_struct_arrays(anyStructArrOff);
    if (!mapPropOff.IsNull())
        containerBuilder.add_map_properties(mapPropOff);
    if (!mapArrOff.IsNull())
        containerBuilder.add_map_arrays(mapArrOff);

    return containerBuilder.Finish();
}

flatbuffers::Offset<fbsvtx::Bucket> VTX::Serialization::ToFlat(flatbuffers::FlatBufferBuilder& builder,
                                                               VTX::Bucket& src) {
    ...
    if (!src.unique_ids.empty()) {
        ...
        for (const auto& s : src.unique_ids)
            offsets.push_back(builder.CreateString(s));
        idsOff = builder.CreateVector(offsets);
    }

    std::vector<flatbuffers::Offset<fbsvtx::PropertyContainer>> entOffsets;
    for (auto& e : src.entities)
        entOffsets.push_back(ToFlat(builder, e));
...
flatbuffers::Offset<fbsvtx::Frame> VTX::Serialization::ToFlat(flatbuffers::FlatBufferBuilder& builder,
                                                              VTX::Frame& src) {
