...
public:
    ...
    void SetEvents(const ReplayReaderEvents& events) override {
        InternalReader.SetEvents(events);
    }

    VTX::FileHeader GetHeader() override {
        return InternalReader.GetFileHeader();
    }

    VTX::FileFooter GetFooter() override {
        return InternalReader.GetFileFooter();
    }

    VTX::ContextualSchema GetContextualSchema() override {
        return InternalReader.GetContextualSchema();
    }
    ...
    int32_t GetTotalFrames() const override {
        return InternalReader.GetTotalFrames();
    }

    const std::vector<ChunkIndexEntry>& GetSeekTable() const override {
        return InternalReader.GetSeekTable();
    }
    ...
    const VTX::Frame* GetFrame(int32_t frame_index) override {
        return InternalReader.GetFramePtr(frame_index);
    }

    const VTX::Frame* GetFrameSync(int frame_index) override {
        return InternalReader.GetFramePtrSync(frame_index);
    }
    ...
    std::vector<VTX::Frame> GetFrameContext(int32_t center_frame, int32_t back_range, int32_t forward_range) override {
    ...
    void InspectChunkHeader(int32_t index) const override {
        InternalReader.InspectChunkHeader(index);
    }

    FrameAccessor CreateAccessor() const override {
        return InternalReader.CreateAccessor();
    }
