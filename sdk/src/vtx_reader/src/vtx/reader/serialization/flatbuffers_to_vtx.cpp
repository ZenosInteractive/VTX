...
void VTX::Serialization::FromFlat(const fbsvtx::PropertyContainer* src, VTX::PropertyContainer& dst) {
    ...
    if (auto v = src->int32_properties())
        dst.int32_properties.assign(v->begin(), v->end());
    if (auto v = src->int64_properties())
        dst.int64_properties.assign(v->begin(), v->end());
    if (auto v = src->float_properties())
        dst.float_properties.assign(v->begin(), v->end());
    if (auto v = src->double_properties())
        dst.double_properties.assign(v->begin(), v->end());

    if (auto v = src->string_properties()) {
        dst.string_properties.reserve(v->size());
        for (auto s : *v)
            dst.string_properties.push_back(s->str());
    }
    ...
    auto unpackStructs = [](auto* fbsVec, auto& nativeVec) {
        ...
        for (size_t i = 0; i < fbsVec->size(); ++i)
            FromFlat(fbsVec->Get(i), nativeVec[i]);
    };
    ...
    auto unpackArray = [](auto* fbsArr, auto& nativeArr) {
        if (!fbsArr)
            return;
        if (fbsArr->data())
            nativeArr.data.assign(fbsArr->data()->begin(), fbsArr->data()->end());
        if (fbsArr->offsets())
            nativeArr.offsets.assign(fbsArr->offsets()->begin(), fbsArr->offsets()->end());
    };
    ...
    if (auto arr = src->bool_arrays()) {
        ...
        if (arr->offsets())
            dst.bool_arrays.offsets.assign(arr->offsets()->begin(), arr->offsets()->end());
    }
    ...
    if (auto arr = src->string_arrays()) {
        if (auto d = arr->data()) {
            ...
            for (auto s : *d)
                dst.string_arrays.data.push_back(s->str());
        }
        if (auto o = arr->offsets())
            dst.string_arrays.offsets.assign(o->begin(), o->end());
    }
    ...
    if (auto v = src->any_struct_properties()) {
        ...
        for (size_t i = 0; i < v->size(); ++i)
            FromFlat(v->Get(i), dst.any_struct_properties[i]);
    }
    ...
    if (auto arr = src->any_struct_arrays()) {
        if (auto d = arr->data()) {
            ...
            for (size_t i = 0; i < d->size(); ++i)
                FromFlat(d->Get(i), dst.any_struct_arrays.data[i]);
        }
        if (auto o = arr->offsets())
            dst.any_struct_arrays.offsets.assign(o->begin(), o->end());
    }
    ...
    if (auto v = src->map_properties()) {
        ...
        for (size_t i = 0; i < v->size(); ++i)
            FromFlat(v->Get(i), dst.map_properties[i]);
    }
    ...
    if (auto arr = src->map_arrays()) {
        if (auto d = arr->data()) {
            ...
            for (size_t i = 0; i < d->size(); ++i)
                FromFlat(d->Get(i), dst.map_arrays.data[i]);
        }
        if (auto o = arr->offsets())
            dst.map_arrays.offsets.assign(o->begin(), o->end());
    }
}
...
void VTX::Serialization::FromFlat(const fbsvtx::Bucket* src, VTX::Bucket& dst) {
    ...
    if (src->unique_ids()) {
        ...
        for (auto s : *src->unique_ids())
            dst.unique_ids.push_back(s->str());
    }
    if (src->entities()) {
        dst.entities.resize(src->entities()->size());
        for (size_t i = 0; i < src->entities()->size(); ++i)
            FromFlat(src->entities()->Get(i), dst.entities[i]);
    }
    ...
    if (src->type_ranges()) {
        ...
        for (size_t i = 0; i < ranges->size(); ++i) {
            ...
            dst.type_ranges.push_back(VTX::EntityRange {r->start_index(), r->count()});
        }
    }
}
