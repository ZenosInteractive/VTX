...
void VTX::FlatBuffersReaderPolicy::ProcessChunkData(int chunk_index,const std::string& compressed, std::stop_token st,std::vector<VTX::Frame>& out_native_frames,std::vector<uint8_t>& out_decompressed_blob,std::vector<std::span<const std::byte>>& out_raw_frames_spans)
 {
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        out_decompressed_blob.insert(out_decompressed_blob.end(), 
...
void VTX::FlatBuffersReaderPolicy::PopulateIndexTable(const FooterType& footer,std::vector<ChunkIndexEntry>& chunk_index_table)
{
...
void VTX::FlatBuffersReaderPolicy::PopulateGameTimes(const FooterType& footer, VTX::GameTime::VTXGameTimes& game_times)
{
    if (footer.times) {
