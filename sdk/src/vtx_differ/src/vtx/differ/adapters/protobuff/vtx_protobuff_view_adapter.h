...
public:
	...
	static std::unique_ptr<FProtobufViewAdapter> CreateRoot(
	    const google::protobuf::DescriptorPool* Pool,
	    const std::string& RootType,
	    const uint8_t* Buffer,
	    size_t Size);

	static std::unique_ptr<FProtobufViewAdapter> FromMessage(
	...
	virtual std::span<const std::byte> GetArrayElementBytes(const VtxDiff::FieldDesc& Field, size_t Index) const override;
	virtual std::span<const std::byte> GetSubArrayBytes(const VtxDiff::FieldDesc& Field, size_t SubIndex) const override;
	virtual std::unique_ptr<IBinaryNodeView> GetNestedStruct(const VtxDiff::FieldDesc& Field) const override;
	virtual std::unique_ptr<IBinaryNodeView> GetArrayElementAsStruct(const VtxDiff::FieldDesc& Field, size_t Index) const override;
	...
	virtual std::unique_ptr<IBinaryNodeView> GetMapValueAsStruct(const VtxDiff::FieldDesc& Field, size_t Index) const override;
