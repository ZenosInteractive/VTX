...
static void PutLE(std::vector<std::byte>& Dst, T Value)
{
    ...
    std::memcpy(buf.data(), &Value, sizeof(T));
        ...
        std::reverse(buf.begin(), buf.end());
    #endif
        for (unsigned char b : buf) Dst.push_back(static_cast<std::byte>(b));
}

static inline void PutF(std::vector<std::byte>& dst, float v) { PutLE(dst, v); }
...
static float SafeGetFloat(const Message& m, const Reflection* r, const FieldDescriptor* f) {
    ...
    const FieldDescriptor* fx = d->FindFieldByName("x"); if (!fx) fx = d->FindFieldByName("r");
    const FieldDescriptor* fy = d->FindFieldByName("y"); if (!fy) fy = d->FindFieldByName("g");
    const FieldDescriptor* fz = d->FindFieldByName("z"); if (!fz) fz = d->FindFieldByName("b");
    ...
    if (fw) PutF(out, SafeGetFloat(m, r, fw));
...
static bool PackTransform(const Message& m, std::vector<std::byte>& out) {
    ...
    auto packVec = [&](const FieldDescriptor* f)->bool {
        if (!f || f->cpp_type() != FieldDescriptor::CPPTYPE_MESSAGE) return false;
    ...
    if (!fT || !fR || !fS) return false;

    if (!packVec(fT)) return false;

    // quat (x,y,z,w)
    {
        const Message& q = r->GetMessage(m, fR);
        if (!PackQuat(q, out)) return false;
    }

    if (!packVec(fS)) return false;
    return true;
}
...
static bool PackKnownStruct(const Message& m, std::vector<std::byte>& out) {
    ...
    if (n == "Vector")     return PackVector(m, out);
...
std::unique_ptr<Message> FProtobufViewAdapter::CloneMessage(const Message& Src)
{
    ...
    const google::protobuf::DescriptorPool* Pool = Desc ? Desc->file()->pool() : google::protobuf::DescriptorPool::generated_pool();
...
// ============================================================
std::optional<FProtobufViewAdapter> FProtobufViewAdapter::CreateRoot(const DescriptorPool* Pool, const std::string& RootType, const uint8_t* Buffer, size_t Size)
{
    if (!Pool || !Buffer || Size == 0) return std::nullopt;
    const google::protobuf::Descriptor* Desc = Pool->FindMessageTypeByName(RootType);
    if (!Desc) return std::nullopt;
    auto OwnedMsg = NewOwnedMessage(Pool, Desc);
    if (!OwnedMsg) return std::nullopt;
    if (!OwnedMsg->ParseFromArray(Buffer, static_cast<int>(Size))) return std::nullopt;
...
{
    const auto* M = GetMsg(); const auto* D = GetDesc(); const auto* R = GetRefl();
    if (!M || !D || !R) return {};
    const auto* Field = FindField(FieldName);
    if (!Field || Field->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE || Field->is_repeated()) return {};
    if (!R->HasField(*M, Field)) return {};
...
{
    if (!Field) return EVTXContainerType::Unknown;
...
{
    if (bFieldsCached) return CachedFields;
    ...
    auto hasData = [&](const google::protobuf::FieldDescriptor* f) -> bool {
        if (!f) return false;
        if (f->is_repeated()) return R->FieldSize(*M, f) > 0;
        
        using FD = google::protobuf::FieldDescriptor;
        switch (f->cpp_type()) {
        case FD::CPPTYPE_MESSAGE: return R->HasField(*M, f);
        case FD::CPPTYPE_STRING:  return !R->GetString(*M, f).empty();
        case FD::CPPTYPE_BOOL:    return R->GetBool(*M, f);
        case FD::CPPTYPE_INT32:   return R->GetInt32(*M, f) != 0;
        case FD::CPPTYPE_UINT32:  return R->GetUInt32(*M, f) != 0;
        case FD::CPPTYPE_INT64:   return R->GetInt64(*M, f) != 0;
        case FD::CPPTYPE_UINT64:  return R->GetUInt64(*M, f) != 0;
        case FD::CPPTYPE_FLOAT:   return R->GetFloat(*M, f) != 0.0f;
        case FD::CPPTYPE_DOUBLE:  return R->GetDouble(*M, f) != 0.0;
        case FD::CPPTYPE_ENUM:    return R->GetEnumValue(*M, f) != 0;
...
std::span<const std::byte> FProtobufViewAdapter::GetFieldBytes(const VtxDiff::FieldDesc& Field) const
{
    ...
    if (!F->is_repeated() && F->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE)
    {
        ...
        {
            case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                PutLE(Scratch, R->GetInt32(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                PutLE(Scratch, R->GetUInt32(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                PutLE(Scratch, R->GetInt64(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                PutLE(Scratch, R->GetUInt64(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                PutLE(Scratch, R->GetFloat(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                PutLE(Scratch, R->GetDouble(*M, F)); break;
            case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
            {
    ...
    if (F->is_repeated() && F->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE)
    {
        ...
        for (int i = 0; i < Count; ++i)
        {
            ...
            {
                case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                    PutLE(Scratch, R->GetRepeatedInt32(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                    PutLE(Scratch, R->GetRepeatedUInt32(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                    PutLE(Scratch, R->GetRepeatedInt64(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                    PutLE(Scratch, R->GetRepeatedUInt64(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                    PutLE(Scratch, R->GetRepeatedFloat(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                    PutLE(Scratch, R->GetRepeatedDouble(*M, F, i)); break;
                case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
                {
...
#if defined(UE_LOG)
    UE_LOG(LogTemp, Verbose, TEXT("[GetArraySize] Field %s size = %d (is_repeated=%d)"),
        *FString(Fd.Name.c_str()), Size, static_cast<int>(F->is_repeated()));
#endif
...
// ============================================================
std::span<const std::byte> FProtobufViewAdapter::GetArrayElementBytes(
    const VtxDiff::FieldDesc& Field, size_t Index) const
{
    const auto* M = GetMsg();
    const auto* D = GetDesc();
    const auto* R = GetRefl();
    ...
    if (F->cpp_type() == google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE)
    {
        const google::protobuf::Message& Nested =
            R->GetRepeatedMessage(*M, F, static_cast<int>(Index));
    ...
    {
        case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
            PutLE(Scratch, R->GetRepeatedInt32(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
            PutLE(Scratch, R->GetRepeatedUInt32(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
            PutLE(Scratch, R->GetRepeatedInt64(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
            PutLE(Scratch, R->GetRepeatedUInt64(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
            PutLE(Scratch, R->GetRepeatedFloat(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
            PutLE(Scratch, R->GetRepeatedDouble(*M, F, static_cast<int>(Index))); break;
        case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
        {
...
// ============================================================
std::span<const std::byte> FProtobufViewAdapter::GetSubArrayBytes(
    const VtxDiff::FieldDesc& Field, size_t SubIndex) const
{
    const auto* M = GetMsg();
    const auto* D = GetDesc();
    const auto* R = GetRefl();
    ...
    UE_LOG(LogTemp, Verbose, TEXT("[GetSubArrayBytes] Msg type = %hs | Field = %hs"),
        UTF8_TO_TCHAR(M->GetDescriptor()->full_name().c_str()),
    ...
    const google::protobuf::Message& SubArrayMsg =
        R->GetRepeatedMessage(*M, F, static_cast<int>(SubIndex));
    ...
    for (int i = 0; i < SubDesc->field_count(); ++i)
    {
        ...
        for (int j = 0; j < InnerCount; ++j)
        {
            ...
            {
                const google::protobuf::Message& ElemMsg =
                    SubRefl->GetRepeatedMessage(SubArrayMsg, InnerField, j);
            ...
            else
            {
                ...
                {
                    case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                        PutLE(Scratch, SubRefl->GetRepeatedInt32(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                        PutLE(Scratch, SubRefl->GetRepeatedUInt32(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
                        PutLE(Scratch, SubRefl->GetRepeatedInt64(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
                        PutLE(Scratch, SubRefl->GetRepeatedUInt64(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
                        PutLE(Scratch, SubRefl->GetRepeatedFloat(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
                        PutLE(Scratch, SubRefl->GetRepeatedDouble(SubArrayMsg, InnerField, j)); break;
                    case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
                    {
...
// ============================================================
FProtobufViewAdapter FProtobufViewAdapter::GetMapValueAsStruct(const VtxDiff::FieldDesc& Field, size_t Index) const
{
    const auto* M = GetMsg(); const auto* D = GetDesc(); const auto* R = GetRefl();
    if (!M || !D || !R) return {};
    const auto* F = D->FindFieldByName(Field.name);
    ...
    if (F->is_map()) {
        ...
        if (!EntryRefl || !ValueField || ValueField->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE) return {};
    ...
    if (Field.type == EVTXContainerType::MapProperties) {
        const auto* ValuesField = D->FindFieldByName("Values");
        if (!ValuesField || !ValuesField->is_repeated() || Index >= static_cast<size_t>(R->FieldSize(*M, ValuesField))) return {};
...
// ============================================================
FProtobufViewAdapter FProtobufViewAdapter::GetArrayElementAsStruct(const VtxDiff::FieldDesc& Field, size_t Index) const
{
    const auto* M = GetMsg(); const auto* D = GetDesc(); const auto* R = GetRefl();
    if (!M || !D || !R) return {};
    const auto* F = D->FindFieldByName(Field.name);
    if (!F || !F->is_repeated() || F->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE) return {};
...
FProtobufViewAdapter FProtobufViewAdapter::GetNestedStruct(const VtxDiff::FieldDesc& Field) const
{
  ...
  // ============================================================================
    ...
    {
        if (RootType.empty() || Buffer.empty()) return std::nullopt;
        ...
        if (!RootDesc) return std::nullopt;
        ...
        if (!SubNames.empty()) View->SetSubArrayNames(SubNames);
        return View;
    }
