...
namespace {
    const reflection::Field* FindFieldByName(const reflection::Object* Obj, std::string_view Name) {
        ...
        for (auto* F : *Obj->fields()) {
            if (F && F->name() && F->name()->string_view() == Name) return F;
        }
        return nullptr;
    }
    ...
    EVTXContainerType InferTypeFromName(std::string_view Name) {
        ...
        if (Name == "bool_properties") return EVTXContainerType::BoolProperties;
        if (Name == "int32_properties") return EVTXContainerType::Int32Properties;
        if (Name == "int64_properties") return EVTXContainerType::Int64Properties;
        if (Name == "float_properties") return EVTXContainerType::FloatProperties;
        if (Name == "double_properties") return EVTXContainerType::DoubleProperties;
        if (Name == "string_properties") return EVTXContainerType::StringProperties;
        if (Name == "transform_properties") return EVTXContainerType::TransformProperties;
        if (Name == "vector_properties") return EVTXContainerType::VectorProperties;
        if (Name == "quat_properties") return EVTXContainerType::QuatProperties;
        if (Name == "range_properties") return EVTXContainerType::RangeProperties;
        if (Name == "byte_array_properties") return EVTXContainerType::ByteArrayProperties;
        
        if (Name == "int32_arrays") return EVTXContainerType::Int32Arrays;
        if (Name == "int64_arrays") return EVTXContainerType::Int64Arrays;
        if (Name == "float_arrays") return EVTXContainerType::FloatArrays;
        if (Name == "double_arrays") return EVTXContainerType::DoubleArrays;
        if (Name == "string_arrays") return EVTXContainerType::StringArrays;
        if (Name == "transform_arrays") return EVTXContainerType::TransformArrays;
        if (Name == "vector_arrays") return EVTXContainerType::VectorArrays;
        if (Name == "quat_arrays") return EVTXContainerType::QuatArrays;
        if (Name == "range_arrays") return EVTXContainerType::RangeArrays;
        if (Name == "bool_arrays") return EVTXContainerType::BoolArrays;

        if (Name == "any_struct_properties") return EVTXContainerType::AnyStructProperties;
        if (Name == "any_struct_arrays" || Name == "entities") return EVTXContainerType::AnyStructArrays;
        if (Name == "map_properties") return EVTXContainerType::MapProperties;
        if (Name == "map_arrays") return EVTXContainerType::MapArrays;
...
void FbSchemaCache::Build(const reflection::Schema* Schema) {
    RawSchema = Schema;
    ObjectCache.clear();
    if (!Schema || !Schema->objects()) return;
    ...
    for (auto* Obj : *Schema->objects()) {
        ...
        for (auto* F : *Obj->fields()) {
            ...
            if (CF.BaseType != reflection::Vector && CF.BaseType != reflection::Obj) {
                CF.ByteSize = flatbuffers::GetTypeSize(static_cast<reflection::BaseType>(static_cast<flatbuffers::ElementaryType>(CF.BaseType)));
            } else if (CF.BaseType == reflection::Obj) {
                const auto* ObjSchema = Schema->objects()->Get(F->type()->index());
                if (ObjSchema && ObjSchema->is_struct()) CF.ByteSize = static_cast<size_t>(ObjSchema->bytesize());
            }

            if (CF.BaseType == reflection::Vector) {
                if (CF.ElementType == reflection::Obj) {
                    const auto* ObjSchema = Schema->objects()->Get(F->type()->index());
                    if (ObjSchema && ObjSchema->is_struct()) CF.ElementByteSize = static_cast<size_t>(ObjSchema->bytesize());
                } else {
                    CF.ElementByteSize = flatbuffers::GetTypeSize(static_cast<reflection::BaseType>(static_cast<flatbuffers::ElementaryType>(CF.ElementType)));
                }
            }
...
const std::vector<FieldDesc>& FbSchemaCache::GetEnumerableFields(const reflection::Object* Obj) const {
    auto It = ObjectCache.find(Obj);
    if (It != ObjectCache.end()) return It->second.EnumerableFields;
    static std::vector<FieldDesc> Empty;
    return Empty;
}


FlatbufferViewAdapter::FlatbufferViewAdapter(const FbSchemaCache* Cache,
                                             const reflection::Object* Object,
                                             const flatbuffers::Table* TablePtr)
    : Cache(Cache), Object(Object), TablePtr(TablePtr) {}
...
std::optional<FlatbufferViewAdapter> FlatbufferViewAdapter::CreateRoot(const FbSchemaCache* Cache, const char* RootObjectName, const uint8_t* Buffer, size_t BufferSize) {    
    if (!Cache || !Cache->RawSchema || !RootObjectName || !Buffer) return std::nullopt;
...
const uint8_t* FlatbufferViewAdapter::GetVectorData(const flatbuffers::Table* Tbl, const reflection::Field* F, size_t& Count, size_t& ElemSize) const {
    Count = 0; ElemSize = 0;
    if (!Tbl || !F || F->type()->base_type() != reflection::Vector) return nullptr;

    auto VecU8 = Tbl->GetPointer<const flatbuffers::Vector<uint8_t>*>(F->offset());
    if (!VecU8) return nullptr;
    Count = VecU8->size();
    ...
    if (F->type()->element() == reflection::Obj) {
        ...
        ElemSize = flatbuffers::GetTypeSize(static_cast<reflection::BaseType>(static_cast<flatbuffers::ElementaryType>(F->type()->element())));
    }
    return VecU8->Data();
}
...
std::span<const std::byte> FlatbufferViewAdapter::GetArrayElementBytes(const FieldDesc& Fd, size_t Index) const {
    ...
    if (CF->SoaDataField) return GetSubArrayBytes(Fd, Index);

    if (!CF->FbField || CF->BaseType != reflection::Vector) return {};

    if (CF->ElementType == reflection::String) {
        auto Vec = TablePtr->GetPointer<const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>*>(CF->FbField->offset());
        if (!Vec || Index >= Vec->size()) return {};
        const flatbuffers::String* S = Vec->Get(Index);
        return S ? std::span<const std::byte>{reinterpret_cast<const std::byte*>(S->c_str()), S->size()} : std::span<const std::byte>{};
    }

    auto VecU8 = TablePtr->GetPointer<const flatbuffers::Vector<uint8_t>*>(CF->FbField->offset());
    if (!VecU8 || Index >= VecU8->size() || CF->ElementByteSize == 0) return {};
...
std::span<const std::byte> FlatbufferViewAdapter::GetSubArrayBytes(const FieldDesc& Fd, size_t SubIndex) const {
    ...
    if (CF->SoaDataField->type()->element() == reflection::String) {
        auto Vec = NestedTable->GetPointer<const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>*>(CF->SoaDataField->offset());
        if (!Vec) return {};

        uint32_t EndElem = (SubIndex + 1 < VecOffsets->size()) ? VecOffsets->Get(static_cast<flatbuffers::uoffset_t>(SubIndex + 1)) : static_cast<uint32_t>(Vec->size());
        if (EndElem < StartElem || EndElem > Vec->size()) return {};
    ...
    size_t DataElemSize = flatbuffers::GetTypeSize(static_cast<reflection::BaseType>(static_cast<flatbuffers::ElementaryType>(CF->SoaDataField->type()->element())));
    
    uint32_t EndElem = (SubIndex + 1 < VecOffsets->size()) ? VecOffsets->Get(static_cast<flatbuffers::uoffset_t>(SubIndex + 1)) : static_cast<uint32_t>(VecData->size() / DataElemSize);
    if (EndElem < StartElem) return {};
...
FlatbufferViewAdapter FlatbufferViewAdapter::GetArrayElementAsStruct(const FieldDesc& Fd, size_t Index) const {
    const auto* CF = Cache->GetCachedField(Object, Fd.name);
    if (!CF || !CF->FbField || CF->FbField->type()->base_type() != reflection::Vector || CF->FbField->type()->element() != reflection::Obj) return {}; // Devuelve vacío

    const auto* NestedObj = Cache->RawSchema->objects()->Get(CF->FbField->type()->index());
    if (!NestedObj || NestedObj->is_struct()) return {};

    auto Vec = TablePtr->GetPointer<const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::Table>>*>(CF->FbField->offset());
    if (!Vec || Index >= Vec->size()) return {};
...
FlatbufferViewAdapter FlatbufferViewAdapter::GetNestedStruct(const FieldDesc& Fd) const {
    const auto* CF = Cache->GetCachedField(Object, Fd.name);
    if (!CF || !CF->FbField || CF->FbField->type()->base_type() != reflection::Obj) return {};
...
FlatbufferViewAdapter FlatbufferViewAdapter::GetMapValueAsStruct(const FieldDesc& Fd, size_t I) const {
    const auto* CF = Cache->GetCachedField(Object, Fd.name);
    if (!CF || !CF->MapValuesField || !CF->MapValuesObj) return {};
    ...
    auto Values = MC->GetPointer<const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::Table>>*>(CF->MapValuesField->offset());
    if (!Values || Values->size() == 0) return {};

    size_t ValIndex = std::min<size_t>(I, Values->size() - 1);
    return FlatbufferViewAdapter(Cache, CF->MapValuesObj, Values->Get(static_cast<flatbuffers::uoffset_t>(ValIndex)));
}

FlatbufferViewAdapter FlatbufferViewAdapter::GetFieldByName(const std::string& FieldName) const {
    FieldDesc fd; fd.name = FieldName; return GetNestedStruct(fd);
}
...
std::string FlatbufferViewAdapter::GetMapKey(const FieldDesc& Fd, size_t I) const {
    ...
    auto Keys = MC->GetPointer<const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>*>(CF->MapKeysField->offset());
...
std::string FlatbufferViewAdapter::GetScalarFieldString(const std::string& FieldName) const {
    ...
    switch (base_type) {
        ...
        case reflection::ULong:
            return std::to_string(TablePtr->GetField<uint64_t>(voffset, static_cast<uint64_t>(FbField->default_integer())));
        case reflection::Long:
            return std::to_string(TablePtr->GetField<int64_t>(voffset, static_cast<int64_t>(FbField->default_integer())));
        case reflection::UInt:
            return std::to_string(TablePtr->GetField<uint32_t>(voffset, static_cast<uint32_t>(FbField->default_integer())));
        case reflection::Int:
            return std::to_string(TablePtr->GetField<int32_t>(voffset, static_cast<int32_t>(FbField->default_integer())));
        ...
        case reflection::Bool:
            return TablePtr->GetField<uint8_t>(voffset, static_cast<uint8_t>(FbField->default_integer())) ? "true" : "false";
        case reflection::UByte:
            return std::to_string(TablePtr->GetField<uint8_t>(voffset, static_cast<uint8_t>(FbField->default_integer())));
        case reflection::Byte:
            return std::to_string(TablePtr->GetField<int8_t>(voffset, static_cast<int8_t>(FbField->default_integer())));
        case reflection::UShort:
            return std::to_string(TablePtr->GetField<uint16_t>(voffset, static_cast<uint16_t>(FbField->default_integer())));
        case reflection::Short:
            return std::to_string(TablePtr->GetField<int16_t>(voffset, static_cast<int16_t>(FbField->default_integer())));
...
std::optional<FlatbufferViewAdapter> FbViewFactory::CreateRoot(std::span<const std::byte> Buffer){
    if (RootName.empty() || Buffer.empty() || !GlobalCache.RawSchema) return std::nullopt;
    return FlatbufferViewAdapter::CreateRoot(&GlobalCache, RootName.c_str(), reinterpret_cast<const uint8_t*>(Buffer.data()), Buffer.size());
}

bool FbViewFactory::InitFromFile(const std::string& Path, const std::string& RootType) { return false; }
