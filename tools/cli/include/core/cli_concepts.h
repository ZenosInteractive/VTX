...
namespace VtxCli {
    ...
    template <typename T>
    concept FormatWriter =
        requires(T writer, bool bool_value, int32_t int32_value, uint32_t uint32_value, int64_t int64_value,
                 uint64_t uint64_value, float float_value, double double_value, const std::string& string_value) {
            //structure
            ...
            //Finelize
            {
                writer.Finalize(string_value)
                } -> std::convertible_to<std::string>; //convertible_to allows to return const string&
        };

    template <typename T>
    concept Transport = requires(T transport, const std::string& string_value) {
