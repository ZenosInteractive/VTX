...
namespace VtxCli {
    ...
    inline std::string FindClosestCommand(const std::string& input,
    ...
    public:
        ...
        void Register()
        {
            ...
            handlers_[name] = [cmd = std::move(cmd)](
                CommandContext& ctx,
        ...
        std::string Dispatch(CommandContext& ctx, const std::string& line)
        {
            ...
            if (it == handlers_.end()) {
                ...
                w.BeginObject()
                 ...
                 .Key("command").WriteString(cmd_name)
                 .Key("error").WriteString("Unknown command. Type 'help' for available commands.");

                // Suggest closest command name
                std::vector<std::string> names;
                names.reserve(handlers_.size());
                for (const auto& [n, _] : handlers_) names.push_back(n);
