...
namespace VtxCli {
    ...
    namespace detail {
        ...
        inline const char* FieldTypeName(VTX::FieldType t) {
            switch (t) {
                case VTX::FieldType::Bool:       return "bool";
                case VTX::FieldType::Int8:       return "int8";
                case VTX::FieldType::Int32:      return "int32";
                case VTX::FieldType::Int64:      return "int64";
                case VTX::FieldType::Float:      return "float";
                case VTX::FieldType::Double:     return "double";
                case VTX::FieldType::String:     return "string";
                case VTX::FieldType::Vector:     return "vector";
                case VTX::FieldType::Quat:       return "quat";
                case VTX::FieldType::Transform:  return "transform";
                case VTX::FieldType::FloatRange: return "float_range";
                case VTX::FieldType::Struct:     return "struct";
                case VTX::FieldType::Enum:       return "enum";
        ...
        inline const char* ContainerTypeName(VTX::FieldContainerType t) {
            switch (t) {
                case VTX::FieldContainerType::Array: return "array";
                case VTX::FieldContainerType::Map:   return "map";
        ...
        inline BucketRef ResolveBucket(const CommandContext& ctx, const VTX::Frame& frame, const std::string& arg) {
            ...
            const auto schema_bucket_names = ExtractBucketNamesFromPropertyMapping(
                ctx.session.GetContextualSchema().property_mapping);
            ...
            if (is_number) {
                ...
                if (idx < buckets.size()) {
                    for (const auto& [bname, bidx] : frame.bucket_map) {
                        if (bidx == idx) return {&buckets[idx], bname, static_cast<int32_t>(idx)};
                    }
        ...
        inline int ResolveEntityIndex(const VTX::Bucket& bucket, const std::string& arg) {
            ...
            if (is_number) {
                try {
                    ...
                    if (idx >= 0 && static_cast<size_t>(idx) < bucket.entities.size()) return idx;
                } catch (...) {}
            }

            for (size_t i = 0; i < bucket.unique_ids.size(); ++i) {
                if (bucket.unique_ids[i] == arg) return static_cast<int>(i);
            }
            return -1;
        }
        ...
        inline std::pair<EntitySelector, size_t> ParseEntitySelector(
            std::span<const std::string> args, size_t start = 0)
        {
        ...
        inline int ResolveEntity(const VTX::Bucket& bucket, const EntitySelector& sel) {
            switch (sel.mode) {
                ...
                case EntitySelector::Mode::ById: {
                    for (size_t i = 0; i < bucket.unique_ids.size(); ++i) {
                        if (bucket.unique_ids[i] == sel.value) return static_cast<int>(i);
                    }
                    return -1;
                }
                default:
    ...
    struct BucketsCommand {
        ...
        void Run(CommandContext& ctx, std::span<const std::string> args, Fmt& w)
        {
            ...
            ResponseOk(w, Name)
                ...
                .Key("count").WriteInt(static_cast<int32_t>(buckets.size()));

            std::unordered_map<size_t, std::string> idx_to_name;
            for (const auto& [bname, bidx] : frame->bucket_map) {
                idx_to_name[bidx] = bname;
            }
            const auto schema_bucket_names = detail::ExtractBucketNamesFromPropertyMapping(
                ctx.session.GetContextualSchema().property_mapping);
            for (size_t i = 0; i < schema_bucket_names.size() && i < buckets.size(); ++i) {
            ...
            for (size_t i = 0; i < buckets.size(); ++i) {
                ...
                w.BeginObject()
                    .Key("index").WriteInt(static_cast<int32_t>(i));
                ...
                w.Key("entity_count").WriteInt(static_cast<int32_t>(bucket.entities.size()))
                    .EndObject();
            }
            w.EndArray();
            EndResponse(w);
    ...
    struct EntitiesCommand {
        ...
        void Run(CommandContext& ctx, std::span<const std::string> args, Fmt& w)
        {
            ...
            ResponseOk(w, Name)
                .Key("bucket_index").WriteInt(ref.index)
                .Key("bucket_name").WriteString(ref.name)
                .Key("count").WriteInt(static_cast<int32_t>(bucket.entities.size()));
            ...
            for (size_t i = 0; i < bucket.entities.size(); ++i) {
                ...
                w.BeginObject()
                    .Key("index").WriteInt(static_cast<int32_t>(i))
                    .Key("unique_id").WriteString(
                        i < bucket.unique_ids.size() ? bucket.unique_ids[i] : "")
                    .Key("entity_type_id").WriteInt(entity.entity_type_id);
    ...
    struct EntityCommand {
        ...
        void Run(CommandContext& ctx, std::span<const std::string> args, Fmt& w)
        {
            ...
            ResponseOk(w, Name)
                .Key("bucket_index").WriteInt(ref.index)
                .Key("bucket_name").WriteString(ref.name)
                .Key("entity_index").WriteInt(idx)
                .Key("unique_id").WriteString(
                    static_cast<size_t>(idx) < ref.bucket->unique_ids.size()
                        ? ref.bucket->unique_ids[idx] : "");
    ...
    struct PropertyCommand {
        ...
        void Run(CommandContext& ctx, std::span<const std::string> args, Fmt& w)
        {
            ...
            ResponseOk(w, Name)
                ...
                .Key("type").WriteString(detail::FieldTypeName(prop_it->second.type_id))
                .Key("container").WriteString(detail::ContainerTypeName(prop_it->second.container_type));
    ...
    struct TypesCommand {
        ...
        void Run(CommandContext& ctx, std::span<const std::string> args, Fmt& w)
        {
            ...
            if (!args.empty()) {
                ...
                ResponseOk(w, Name)
                    .Key("type_name").WriteString(sc.name)
                    .Key("type_id").WriteInt(name_it->second)
                    .Key("property_count").WriteInt(static_cast<int32_t>(sc.properties.size()));

                w.Key("properties");
                w.BeginArray();
                for (const auto& prop_view : sc.GetPropertiesInOrder()) {
                    w.BeginObject()
                        .Key("name").WriteString(std::string(prop_view.name));
                    if (prop_view.address) {
                        w.Key("type").WriteString(detail::FieldTypeName(prop_view.address->type_id))
                         .Key("container").WriteString(detail::ContainerTypeName(prop_view.address->container_type));
            ...
            ResponseOk(w, Name)
                .Key("count").WriteInt(static_cast<int32_t>(cache.name_to_id.size()));
            ...
            for (const auto& [name, id] : cache.name_to_id) {
                ...
                w.BeginObject()
                    .Key("name").WriteString(name)
                    .Key("type_id").WriteInt(id)
                    .Key("property_count").WriteInt(
                        struct_it != cache.structs.end()
                            ? static_cast<int32_t>(struct_it->second.properties.size()) : 0)
                    .EndObject();
            }
            w.EndArray();
            EndResponse(w);
