...
namespace VtxCli {
    ...
    Fmt& ResponseOk(Fmt& writer, std::string_view command_name) {
        writer.BeginObject()
            ...
            .WriteString("ok")
            .Key("command")
            .WriteString(std::string(command_name))
            .Key("data");
    ...
    template <FormatWriter Fmt>
    Fmt& ResponseErrorHint(Fmt& writer, std::string_view command_name, const std::string& message,
    ...
    inline std::string FindClosestMatch(const std::string& input, const std::vector<std::string>& candidates,
                                        size_t threshold = 3) {
        std::string best;
    ...
    inline std::string SuggestProperty(const VTX::PropertyAddressCache& cache, int32_t entity_type_id,
    ...
    inline std::string SuggestTypeName(const VTX::PropertyAddressCache& cache, const std::string& input) {
        std::vector<std::string> names;
