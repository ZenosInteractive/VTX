...
int main(int argc, char* argv[]) {
    ...
    if (port > 0) {
        VtxCli::CliEngine<VtxCli::JsonFormat, VtxCli::TcpTransport> engine(static_cast<uint16_t>(port), verbose);
        engine.Run();
    } else {
        VtxCli::CliEngine<VtxCli::JsonFormat, VtxCli::StdioTransport> engine(verbose);
        engine.Run();
    }
