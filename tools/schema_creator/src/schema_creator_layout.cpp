...
bool SaveSchemaFromMenu(SchemaCreatorSession& session, bool force_save_as) {
    ...
    const std::string dialog_title = force_save_as
        ? (next_generation_mode ? "Save next generation schema as..." : "Save schema as...")
        : (next_generation_mode ? "Save next generation schema" : "Save schema");

    auto save_dialog = pfd::save_file(
        dialog_title,
        BuildSaveSuggestionPath(session, apply_next_generation_rules),
...
void BuildDockLayout(ImGuiID dockspace_id, const ImVec2& workspace_size) {
    ...
    ImGuiID dock_top_right_id = ImGui::DockBuilderSplitNode(dock_top_left_id, ImGuiDir_Right, 0.30f, nullptr, &dock_top_left_id);
...
SchemaCreatorLayout::SchemaCreatorLayout(
    const std::shared_ptr<SchemaCreatorSession>& session,
                ...
                if (recent_files.empty()) {
                    ...
                    for (const auto& recent_path : recent_files) {
                        if (ImGui::MenuItem(recent_path.c_str())) {
                            ...
                            SetStatus(
                                loaded ? "Schema loaded: " + recent_path : "Failed to load schema: " + recent_path,
                                !loaded);
                        }
                    }
                }
            ...
            if (ImGui::MenuItem("Validate")) {
                ...
                SetStatus(
                    session_->GetValidationReport().is_valid
                        ? "Validation completed (no blocking errors)."
