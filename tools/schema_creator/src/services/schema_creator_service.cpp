...
namespace {
    ...
    void RecomputeFieldIndices(VtxServices::SchemaStructDoc& schema_struct) {
        ...
        for (auto& field : schema_struct.fields) {
            const std::string index_key =
                CanonicalizeTypeId(field.type_id) + "|" + CanonicalizeContainerType(field.container_type);
    ...
    bool AreSignaturesEqual(const FieldSignature& lhs, const FieldSignature& rhs) {
        return lhs.type_id == rhs.type_id && lhs.struct_type == rhs.struct_type && lhs.key_id == rhs.key_id &&
               lhs.container_type == rhs.container_type && lhs.index == rhs.index;
    }
    ...
    bool AreMetadataEqual(const VtxServices::SchemaFieldDoc& lhs, const VtxServices::SchemaFieldDoc& rhs) {
        return lhs.meta.type == rhs.meta.type && lhs.meta.key_type == rhs.meta.key_type &&
               lhs.meta.category == rhs.meta.category && lhs.meta.display_name == rhs.meta.display_name &&
               lhs.meta.tooltip == rhs.meta.tooltip && lhs.meta.default_value == rhs.meta.default_value &&
               lhs.meta.version == rhs.meta.version && lhs.meta.fixed_array_dim == rhs.meta.fixed_array_dim;
    }

    // Finds a field by logical name in a schema struct.
    const VtxServices::SchemaFieldDoc* FindFieldByName(const VtxServices::SchemaStructDoc& schema_struct,
    ...
    void AddValidationIssue(VtxServices::ValidationReport& report, VtxServices::ValidationSeverity severity,
                            const std::string& message, const std::string& struct_name = {},
                            const std::string& field_name = {}) {
    ...
    void AddEvolutionIssue(VtxServices::EvolutionReport& report, VtxServices::ValidationSeverity severity,
                           VtxServices::EvolutionChangeKind kind, const std::string& message,
    ...
    std::unordered_map<std::string, const VtxServices::SchemaStructDoc*>
    BuildStructLookup(const VtxServices::SchemaDocument& document) {
    ...
    std::unordered_map<std::string, const VtxServices::SchemaFieldDoc*>
    BuildFieldLookup(const VtxServices::SchemaStructDoc& schema_struct) {
...
namespace VtxServices {
    ...
    bool SchemaCreatorService::LoadFromFile(const std::string& path, SchemaDocument& out_document,
    ...
    bool SchemaCreatorService::SaveToFile(const SchemaDocument& document, const std::string& path,
                                          std::string& out_error) {
        out_error.clear();
    ...
    bool SchemaCreatorService::ParseFromJsonString(const std::string& raw_json, SchemaDocument& out_document,
                                                   std::string& out_error) {
        out_error.clear();
        json root;
        try {
    ...
    std::string SchemaCreatorService::SerializeToJson(const SchemaDocument& document, bool pretty) {
        ...
        for (const auto& schema_struct : normalized.structs) {
            ...
            for (const auto& field : schema_struct.fields) {
                ...
                meta_json["keyType"] =
                    field.meta.key_type.empty() ? InferMetaKeyTypeFromKeyId(field.key_id) : field.meta.key_type;
    ...
    ValidationReport SchemaCreatorService::ValidateSchema(const SchemaDocument& document) {
        ...
        for (const auto& schema_struct : normalized.structs) {
            ...
            if (!struct_names.insert(schema_struct.struct_name).second) {
                AddValidationIssue(report, ValidationSeverity::Error,
            ...
            for (const auto& field : schema_struct.fields) {
                if (field.name.empty()) {
                    AddValidationIssue(report, ValidationSeverity::Error, "Field name is empty.",
                                       schema_struct.struct_name);
                    continue;
                }
                if (!field_names.insert(field.name).second) {
                    AddValidationIssue(report, ValidationSeverity::Error, "Duplicate field name: '" + field.name + "'.",
                                       schema_struct.struct_name, field.name);
                }

                const std::string container = NormalizeContainerType(field.container_type);
                if (container.empty()) {
                    AddValidationIssue(report, ValidationSeverity::Warning,
                                       "containerType should be one of: 'None', 'Array', 'Map'.",
                                       schema_struct.struct_name, field.name);
                }

                if (CanonicalizeTypeId(field.type_id) == "Struct" && field.struct_type.empty()) {
                    AddValidationIssue(report, ValidationSeverity::Error,
                                       "Field with typeId 'Struct' requires 'structType'.", schema_struct.struct_name,
                                       field.name);
                }

                if (container == "map" && CanonicalizeTypeId(field.key_id) == "None") {
                    AddValidationIssue(report, ValidationSeverity::Warning, "Map field should define 'keyId'.",
                                       schema_struct.struct_name, field.name);
                }

                if (CanonicalizeTypeId(field.type_id) == "None") {
                    AddValidationIssue(report, ValidationSeverity::Error, "Field typeId cannot be empty/None.",
                                       schema_struct.struct_name, field.name);
                } else if (!IsKnownTypeToken(field.type_id)) {
                    AddValidationIssue(report, ValidationSeverity::Warning,
                                       "Unknown field typeId token '" + field.type_id + "'.", schema_struct.struct_name,
                                       field.name);
                }
            }
        }
    ...
    EvolutionReport SchemaCreatorService::BuildEvolutionReport(const SchemaDocument& baseline,
                                                               const SchemaDocument& candidate) {
        ...
        for (const auto& [struct_name, candidate_struct] : candidate_structs) {
            ...
            if (baseline_structs.find(struct_name) == baseline_structs.end()) {
                AddEvolutionIssue(report, ValidationSeverity::Info, EvolutionChangeKind::StructAdded, "Struct added.",
                                  struct_name);
            }
        }
        ...
        for (const auto& [struct_name, baseline_struct] : baseline_structs) {
            if (candidate_structs.find(struct_name) == candidate_structs.end()) {
                AddEvolutionIssue(report, ValidationSeverity::Error, EvolutionChangeKind::StructRemoved,
                                  "Struct removed. Breaking change.", struct_name);
                continue;
            }
            ...
            for (const auto& [field_name, candidate_field] : candidate_fields) {
                if (baseline_fields.find(field_name) == baseline_fields.end()) {
                    AddEvolutionIssue(report, ValidationSeverity::Info, EvolutionChangeKind::FieldAdded,
                                      "Field added (compatible additive change).", struct_name, field_name);
                    continue;
                }
                ...
                if (!AreSignaturesEqual(BuildFieldSignature(*baseline_field), BuildFieldSignature(*candidate_field))) {
                    AddEvolutionIssue(
                        report, ValidationSeverity::Error, EvolutionChangeKind::FieldChanged,
                ...
                if (!AreMetadataEqual(*baseline_field, *candidate_field)) {
                    AddEvolutionIssue(report, ValidationSeverity::Warning, EvolutionChangeKind::FieldMetaChanged,
                                      "Field metadata changed (non-breaking, review downstream usage).", struct_name,
                                      field_name);
                }
            }
            ...
            for (const auto& [field_name, baseline_field] : baseline_fields) {
                ...
                if (candidate_fields.find(field_name) == candidate_fields.end()) {
                    AddEvolutionIssue(report, ValidationSeverity::Error, EvolutionChangeKind::FieldRemoved,
                                      "Field removed. Breaking change.", struct_name, field_name);
                }
            }
        }
    ...
    void SchemaCreatorService::ApplyNextGenerationVersion(SchemaDocument& candidate, const SchemaDocument& baseline) {
        ...
        const int32_t next_version = std::max(ComputeNextGenerationVersion(normalized_baseline),
                                              ComputeNextGenerationVersion(normalized_candidate)) +
                                     1;
        const auto baseline_structs = BuildStructLookup(normalized_baseline);
        ...
        for (auto& candidate_struct : candidate.structs) {
            ...
            for (auto& field : candidate_struct.fields) {
                ...
                const bool should_bump = (baseline_field == nullptr) || (normalized_candidate_field == nullptr) ||
                                         !AreSignaturesEqual(BuildFieldSignature(*baseline_field),
                                                             BuildFieldSignature(*normalized_candidate_field)) ||
