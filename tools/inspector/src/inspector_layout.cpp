...
CommandResult BuildOpenReplayResult(const std::string& selected_path, bool success) {
    ...
    result.log_message = success
        ? "Replay opened: " + selected_path
        : "Failed to open replay: " + selected_path;
    return result;
}
...
InspectorLayout::InspectorLayout(
    const std::shared_ptr<InspectorSession>& session,
...
void InspectorLayout::OnRender() {
    ...
    if (force_layout_reset_ || ImGui::DockBuilderGetNode(dockspace_id) == nullptr) {
        ...
        ImGuiID dock_id_left = ImGui::DockBuilderSplitNode(dock_main_id, ImGuiDir_Left, layout_spec.left_ratio, nullptr, &dock_main_id);
        ImGuiID dock_id_left_top = ImGui::DockBuilderSplitNode(dock_id_left, ImGuiDir_Up, layout_spec.left_top_ratio, nullptr, &dock_id_left);
        ImGuiID dock_id_left_bottom = dock_id_left;

        ImGuiID dock_id_bottom = ImGui::DockBuilderSplitNode(dock_main_id, ImGuiDir_Down, layout_spec.bottom_ratio, nullptr, &dock_main_id);
        ImGuiID dock_id_details = ImGui::DockBuilderSplitNode(
            dock_main_id,
            ImGuiDir_Right,
            layout_spec.top_right_details_ratio,
            nullptr,
            &dock_main_id);
    ...
    if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("File")) {
            ...
            if (ImGui::MenuItem("Open VTX Replay...")) {
                auto f = pfd::open_file("Select a VTX Replay", ".", 
        ...
        if (ImGui::BeginMenu("Analysis", session_->HasLoadedReplay())) {
            if (ImGui::MenuItem("Entity LifeTime")) {
                analysis_windows_.push_back(
                    AnalysisWindowFactory::CreateEntityLifeTimeWindow(
                        session_, ++analysis_window_counter_));
            }
            if (ImGui::MenuItem("Unique Properties")) {
                analysis_windows_.push_back(
                    AnalysisWindowFactory::CreateUniquePropertiesWindow(
                        session_, ++analysis_window_counter_));
            }
            if (ImGui::MenuItem("Track Property")) {
                analysis_windows_.push_back(
                    AnalysisWindowFactory::CreateTrackPropertyWindow(
                        session_, ++analysis_window_counter_));
            }
            ImGui::EndMenu();
        }
