...
void EntityDetailsWindow::DrawPropertiesPanel(const VtxServices::EntityInspectorViewModel& view_model,
    ...
    if (!view_model.empty_properties_message.empty() && view_model.properties.roots.empty()) {
        const bool is_missing_entity =
            !state.selected_entity_id.empty() &&
    ...
    if (show_schema_names && !view_model.header.missing_field_names.empty()) {
        ...
        ImGui::TextColored(ImVec4(1.0f, 0.55f, 0.3f, 1.0f),
                           "Missing Fields (%zu):", view_model.header.missing_field_names.size());
...
void EntityDetailsWindow::RenderPropertyNode(const VtxServices::EntityPropertyNode& node,
                                             const VTX::PropertyAddressCache* global_cache, bool show_schema_names) {
    if (node.is_property) {
        ...
        if (ImGui::Selectable(display_text.c_str())) {
            const auto effect =
                VtxServices::EntityInspectorViewService::BuildPropertyActivateEffect(node, global_cache);
    ...
    if (ImGui::IsItemHovered()) {
        ...
        if (effect.request_schema_focus) {
            ...
            if (!effect.schema_property_name.empty()) {
                ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "Schema Name: %s",
                                   effect.schema_property_name.c_str());
            } else {
    ...
    if (!node.children.empty() && open) {
        ...
        if (node.schema_type == VTX::FieldType::Int8 && node.schema_container_type == VTX::FieldContainerType::Array) {
            ...
            const bool should_extra_indent =
...
void EntityDetailsWindow::DrawHexViewerForNode(const VtxServices::EntityPropertyNode& node) {
    ...
    const char* type_names[] = {"8-bit Integer",  "16-bit Integer", "32-bit Integer",
                                "64-bit Integer", "32-bit Float",   "64-bit Double"};
    ...
    if (data_type == 0) {
        bytes_per_item = 1;
        item_width = (display_base == 0) ? 3 : (display_base == 1) ? 4 : 5;
    } else if (data_type == 1) {
        bytes_per_item = 2;
        item_width = (display_base == 0) ? 5 : (display_base == 1) ? 6 : 7;
    } else if (data_type == 2) {
        bytes_per_item = 4;
        item_width = (display_base == 0) ? 9 : (display_base == 1) ? 11 : 12;
    } else if (data_type == 3) {
        bytes_per_item = 8;
        item_width = (display_base == 0) ? 17 : 21;
    } // Signed/Unsigned 64-bit occupy max ~20 + space
    else if (data_type == 4) {
        bytes_per_item = 4;
        item_width = 11;
    } else if (data_type == 5) {
        ...
        for (size_t j = 0; j < bytes_per_line; j += bytes_per_item) {
            if (i + j + bytes_per_item <= raw_bytes.size()) {
                ...
                case 0: { // 8-bit
                    uint8_t v = *data_ptr;
                    if (display_base == 0)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%02X ", v);
                    else if (display_base == 1)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%3u ", v);
                    else
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%4d ", (int8_t)v);
                    break;
                }
                case 1: { // 16-bit
                    uint16_t v;
                    ...
                    if (display_base == 0)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%04X ", v);
                    else if (display_base == 1)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%5u ", v);
                    else
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%6d ", (int16_t)v);
                    break;
                }
                case 2: { // 32-bit
                    uint32_t v;
                    std::memcpy(&v, data_ptr, sizeof(v));
                    if (display_base == 0)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%08X ", v);
                    else if (display_base == 1)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%10u ", v);
                    else
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%11d ", (int32_t)v);
                    break;
                }
                case 3: { // 64-bit
                    uint64_t v;
                    std::memcpy(&v, data_ptr, sizeof(v));
                    if (display_base == 0)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%016llX ", (unsigned long long)v);
                    else if (display_base == 1)
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%20llu ", (unsigned long long)v);
                    else
                        ptr += snprintf(ptr, 512 - (ptr - line_buffer), "%20lld ", (long long)v);
                    break;
                }
                case 4: { // 32-bit Float
                    float v;
