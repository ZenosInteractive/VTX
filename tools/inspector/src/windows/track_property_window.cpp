...
TrackPropertyWindow::TrackPropertyWindow(std::shared_ptr<InspectorSession> session, int instance_id)
...
void TrackPropertyWindow::DrawConfigPanel() {
    ...
    ImGui::InputTextWithHint("##PropertyNames", "Property names to track (comma-separated, e.g. Health,Score)",
    ...
    if (ImGui::Button("Run Analysis", ImVec2(140.0f, 0.0f))) {
        ...
        scanner_.Start(VtxServices::AnalysisScanner::ScanType::TrackProperty, config, reader, cache, footer);
    }
    if (!can_run)
        ImGui::EndDisabled();
