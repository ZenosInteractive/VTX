...
UniquePropertiesWindow::UniquePropertiesWindow(
...
void UniquePropertiesWindow::DrawConfigPanel() {
    ...
    ImGui::InputTextWithHint("##PropertyNames",
        "Property names (comma-separated, e.g. Identifier,ClassIdentifier)",
    ...
    if (ImGui::Button("Run Analysis", ImVec2(140.0f, 0.0f))) {
        ...
        scanner_.Start(VtxServices::AnalysisScanner::ScanType::UniqueProperties,
        ...
        for (size_t g = 0; g < result.groups.size(); ++g) {
            ...
            if (!filter_query.empty()) {
                ...
                for (const auto& val : group.property_values) {
                    if (ContainsCaseInsensitive(val, filter_query)) { matches = true; break; }
                }
                ...
                if (!matches) {
                    for (const auto& uid : group.contributing_uids) {
                        if (ContainsCaseInsensitive(uid, filter_query)) { matches = true; break; }
                    }
                }
                if (!matches) continue;
            }
