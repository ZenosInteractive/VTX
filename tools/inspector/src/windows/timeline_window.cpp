...
namespace {
    ...
    float ComputeCenteredScroll(const VtxServices::TimelineBarState& timeline_bar_state, int frame_index,
                                float view_width) {
        return VtxServices::TimelineViewService::ComputeCenteredScroll(
            frame_index, ComputeItemFullWidth(timeline_bar_state), view_width);
    }
    ...
    TimelineStripViewModel BuildStripViewModel(VtxServices::TimelineBarState& timeline_bar_state, int total_frames,
        ...
        if (is_window_hovered && ctrl_down && wheel != 0.0f) {
            ...
            view_model.desired_scroll_x = VtxServices::TimelineViewService::ComputeZoomedScroll(
                wheel, mouse_x_in_child, scroll_x, timeline_bar_state);
        ...
        const auto range = VtxServices::TimelineViewService::ComputeVisibleRange(working_scroll_x, view_width,
                                                                                 total_frames, timeline_bar_state);
        ...
        const int hovered_frame_idx = VtxServices::TimelineViewService::ResolveHoveredFrame(
            mouse_x, mouse_y, origin_x, origin_y, timeline_height, range);
...
void TimelineWindow::DrawFrameStripTimeline(int total_frames) {
    ...
    const bool child_visible =
        ImGui::BeginChild("FrameTimelineScroll", ImVec2(avail_width, timeline_height + ImGui::GetStyle().ScrollbarSize),
    ...
    if (child_visible) {
        ...
        child_draw_list->AddRectFilled(p, ImVec2(p.x + strip_vm.total_content_width, p.y + timeline_height),
