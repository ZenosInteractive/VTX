...
std::string BuildActiveChunkWindowText(
    const std::vector<VTX::ChunkIndexEntry>& seek_table,
...
std::string BuildLoadedChunkWindowText(
    const std::vector<int32_t>& loaded_chunks,
    const std::vector<VTX::ChunkIndexEntry>& seek_table,
    int active_chunk_index,
    int backward_count,
    int forward_count) {
...
void ReaderInspectorWindow::DrawContent() {
    ...
    const auto vm = VtxServices::ReaderChunkStateService::BuildProfileViewModel(snapshot, ImGui::GetTime(), profile_cache_);
    ...
    if (!rows.empty()) {
        ...
        const std::string loaded_chunk_window = BuildLoadedChunkWindowText(
            ...
            seek_table,
            active_chunk_index,
            cache_window_backward_,
            cache_window_forward_);
    ...
    if (ImGui::BeginTable("ChunksTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
        ...
        for (const auto& row : rows) {
            ...
            if (row.highlighted) {
                ImGui::TextColored(
                    row.state_label.rfind("Loaded", 0) == 0 ? ImVec4(0.2f, 1.0f, 0.2f, 1.0f) : ImVec4(1.0f, 0.8f, 0.2f, 1.0f),
                    "%s",
...
void ReaderInspectorWindow::ApplyCacheWindowIfNeeded(VTX::IVtxReaderFacade* reader, bool force_apply) {
    ...
    reader->SetCacheWindow(
        static_cast<uint32_t>(cache_window_backward_),
        static_cast<uint32_t>(cache_window_forward_));
