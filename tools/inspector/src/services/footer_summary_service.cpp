...
namespace {
    ...
    std::string FormatDurationLabel(float duration_seconds) {
        ...
        if (hours > 0) {
            std::snprintf(units_buffer, sizeof(units_buffer), "%lldh %lldm %llds %lldms", static_cast<long long>(hours),
                          static_cast<long long>(minutes), static_cast<long long>(seconds),
                          static_cast<long long>(milliseconds));
        } else if (minutes > 0) {
            std::snprintf(units_buffer, sizeof(units_buffer), "%lldm %llds %lldms", static_cast<long long>(minutes),
                          static_cast<long long>(seconds), static_cast<long long>(milliseconds));
        } else {
            std::snprintf(units_buffer, sizeof(units_buffer), "%llds %lldms", static_cast<long long>(seconds),
                          static_cast<long long>(milliseconds));
        }
...
namespace VtxServices {
    ...
    FileInfoViewModel FooterSummaryService::BuildFileInfoViewModel(const std::string& file_path, float file_size_mb,
        ...
        version_section.rows = {
            FileInfoRow {"Format Version", "v" + std::to_string(header.version.format_major) + "." +
                                               std::to_string(header.version.format_minor)},
    ...
    std::string FooterSummaryService::FormatChunkSize(uint32_t chunk_size_bytes) {
        ...
        if (chunk_size_bytes >= 1024 * 1024) {
            std::snprintf(buffer, sizeof(buffer), "%.2f MB",
                          static_cast<float>(chunk_size_bytes) / (1024.0f * 1024.0f));
    ...
    FooterSummaryService::BuildPropertySchemaSummaryRows(const VTX::PropertySchema& prop_schema) {
        return {
    ...
    std::vector<ChunkIndexRow>
    FooterSummaryService::BuildChunkRows(const std::vector<VTX::ChunkIndexEntry>& chunk_index) {
    ...
    std::vector<TimelineEventRow>
    FooterSummaryService::BuildTimelineEventRows(const std::vector<VTX::TimelineEvent>& events) {
    ...
    ReplayTimeViewModel FooterSummaryService::BuildReplayTimeViewModel(const VTX::ReplayTimeData& time_data,
        ...
        auto append_tick_tab = [&view_model, time_display_format](const std::string& label,
                                                                  const std::string& value_column_label,
                                                                  const auto& values, const auto& formatter) {
        ...
        auto append_raw_tab = [&view_model](const std::string& label, const std::string& value_column_label,
                                            const auto& values) {
            ReplayTimeTabViewModel tab;
            tab.label = label;
            tab.value_column_label = value_column_label;
    ...
    FooterTableViewModel
    FooterSummaryService::BuildChunkTableViewModel(const std::vector<VTX::ChunkIndexEntry>& chunk_index) {
        ...
        for (const auto& row : rows) {
            ...
            vm_row.cells = {
                FooterTableCell {std::to_string(row.chunk_index)},      FooterTableCell {row.frame_range},
                FooterTableCell {row.file_offset, true, false},         FooterTableCell {row.chunk_size},
    ...
    FooterTableViewModel
    FooterSummaryService::BuildTimelineEventsTableViewModel(const std::vector<VTX::TimelineEvent>& events) {
        FooterTableViewModel view_model;
