...
TimelineClockSpan TimelineViewService::BuildTimelineClockSpan(
    int current_frame,
    int total_frames,
    ...
    return TimelineClockSpan{
        .current = ToClockTime(current_time_sec),
        .total = ToClockTime(duration_seconds),
        .fps = fps
    };
}
...
TimelineVisibleRange TimelineViewService::ComputeVisibleRange(
    float scroll_x,
    float view_width,
    int total_frames,
...
float TimelineViewService::ComputeZoomedScroll(
    float wheel,
    float mouse_x_in_child,
    float scroll_x,
...
int TimelineViewService::ResolveHoveredFrame(
    float mouse_x,
    float mouse_y,
    float origin_x,
    float origin_y,
    float timeline_height,
    const TimelineVisibleRange& range) {
    for (int i = range.first_visible_idx; i < range.last_visible_idx; ++i) {
        ...
        if (mouse_x >= x_start && mouse_x < x_end &&
            mouse_y >= origin_y && mouse_y <= y_end) {
            return i;
        }
    }
    return -1;
