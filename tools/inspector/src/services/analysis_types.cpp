    ...
    const VTX::PropertyAddress& addr)
{
    ...
    switch (addr.type_id) {
        case VTX::FieldType::Bool:
            if (idx >= entity.bool_properties.size()) return "";
            return entity.bool_properties[idx] ? "true" : "false";
        case VTX::FieldType::Int32:
        case VTX::FieldType::Enum:
            if (idx >= entity.int32_properties.size()) return "";
            return std::to_string(entity.int32_properties[idx]);
        case VTX::FieldType::Int64:
            if (idx >= entity.int64_properties.size()) return "";
            return std::to_string(entity.int64_properties[idx]);
        case VTX::FieldType::Float:
            if (idx >= entity.float_properties.size()) return "";
            return std::format("{:.6g}", entity.float_properties[idx]);
        case VTX::FieldType::Double:
            if (idx >= entity.double_properties.size()) return "";
            return std::format("{:.6g}", entity.double_properties[idx]);
        case VTX::FieldType::String:
            if (idx >= entity.string_properties.size()) return "";
            return entity.string_properties[idx];
        case VTX::FieldType::Vector:
            if (idx >= entity.vector_properties.size()) return "";
            return std::format("({:.2f},{:.2f},{:.2f})",
                entity.vector_properties[idx].x,
                entity.vector_properties[idx].y,
                entity.vector_properties[idx].z);
        case VTX::FieldType::Quat:
            if (idx >= entity.quat_properties.size()) return "";
            return std::format("({:.4f},{:.4f},{:.4f},{:.4f})",
                entity.quat_properties[idx].x,
                entity.quat_properties[idx].y,
                entity.quat_properties[idx].z,
                entity.quat_properties[idx].w);
        case VTX::FieldType::Transform:
            if (idx >= entity.transform_properties.size()) return "";
            return std::format("T({:.2f},{:.2f},{:.2f})",
                entity.transform_properties[idx].translation.x,
                entity.transform_properties[idx].translation.y,
                entity.transform_properties[idx].translation.z);
        case VTX::FieldType::FloatRange:
            if (idx >= entity.range_properties.size()) return "";
            return std::format("[{:.2f}-{:.2f}]",
                entity.range_properties[idx].min,
                entity.range_properties[idx].max);
        default:
            return "";
    }
}
...
std::string FormatSingleGameTimeRange(const FrameRange& range) {
    return std::format("[{}-{}]",
        GameTimeToSeconds(range.start_game_time),
        GameTimeToSeconds(range.end_game_time));
}
...
bool ExportEntityLifeTimeCsv(const std::string& path, const EntityLifeTimeResult& result) {
    ...
    for (const auto& entry : result.entries) {
        file << CsvEscape(entry.unique_id) << ","
             << CsvEscape(entry.type_name) << ","
             << entry.type_id << ","
             << CsvEscape(FormatFrameRanges(entry.ranges)) << ","
             << CsvEscape(FormatGameTimeRanges(entry.ranges)) << "\n";
    }
...
bool ExportTrackPropertyCsv(const std::string& path, const TrackPropertyResult& result) {
    ...
    for (const auto& span : result.spans) {
        file << CsvEscape(span.unique_id) << ","
             << CsvEscape(span.property_name) << ","
             << CsvEscape(span.value) << ","
