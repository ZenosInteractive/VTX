...
const VTX::Bucket* ResolveScanBucket(
    const VTX::Frame& frame,
    const AnalysisScanner::ScanConfig& config)
{
    const auto& buckets = frame.GetBuckets();
    if (config.bucket_index >= 0 &&
        static_cast<size_t>(config.bucket_index) < buckets.size()) {
...
void AnalysisScanner::RunEntityLifeTime(
    const ScanConfig& config,
    VTX::IVtxReaderFacade* reader,
    const VTX::PropertyAddressCache& cache,
    const VTX::FileFooter& footer)
{
    ...
    for (int32_t f = config.start_frame; f <= config.end_frame; ++f) {
        ...
        for (size_t i = 0; i < bucket->unique_ids.size(); ++i) {
            ...
            if (!PassesUidFilter(config.filter_unique_ids, uid)) continue;
    ...
    if (frames_with_bucket == 0) {
        lifetime_result_.error_message = std::format(
            "Selected bucket {} was not present in frames {}-{}.",
            BuildBucketDebugLabel(config),
            config.start_frame,
            config.end_frame);
...
void AnalysisScanner::RunUniqueProperties(
    const ScanConfig& config,
    VTX::IVtxReaderFacade* reader,
    const VTX::PropertyAddressCache& cache,
    const VTX::FileFooter& footer)
{
    ...
    std::sort(unique_props_result_.groups.begin(), unique_props_result_.groups.end(),
        [](const UniquePropertiesGroup& a, const UniquePropertiesGroup& b) {
    ...
    progress_.store(1.0f);
    if (frames_with_bucket == 0) {
        unique_props_result_.error_message = std::format(
            "Selected bucket {} was not present in frames {}-{}.",
            BuildBucketDebugLabel(config),
            config.start_frame,
            config.end_frame);
        ...
        unique_props_result_.error_message = "None of the matched entities contained all requested scalar properties.";
    }
...
void AnalysisScanner::RunTrackProperty(
    const ScanConfig& config,
    VTX::IVtxReaderFacade* reader,
    const VTX::PropertyAddressCache& cache,
    const VTX::FileFooter& footer)
{
    ...
    for (int32_t f = config.start_frame; f <= config.end_frame; ++f) {
        ...
        for (size_t i = 0; i < bucket->entities.size(); ++i) {
            ...
            for (const auto& prop_name : config.property_names) {
                ...
                if (prop_it == sc_it->second.properties.end()) continue;
    ...
    std::sort(track_prop_result_.spans.begin(), track_prop_result_.spans.end(),
        [](const PropertyValueSpan& a, const PropertyValueSpan& b) {
            ...
            if (a.property_name != b.property_name) return a.property_name < b.property_name;
    ...
    progress_.store(1.0f);
    if (frames_with_bucket == 0) {
        track_prop_result_.error_message = std::format(
            "Selected bucket {} was not present in frames {}-{}.",
            BuildBucketDebugLabel(config),
            config.start_frame,
            config.end_frame);
