...
namespace VtxServices {
    ...
    namespace {
        ...
        void BuildPropertyNodesRecursive(const VTX::PropertyContainer& pc,
                                         const VTX::PropertyAddressCache* global_cache, const std::string& id_prefix,
        ...
        Node MakePropertyNode(const std::string& id, const std::string& label, const std::string& value,
                              const std::string& raw_value, const std::string& struct_name, int schema_index,
                              VTX::FieldType schema_type, VTX::FieldContainerType schema_container_type) {
            Node node;
            node.id = id;
            node.label = label;
            node.value = value;
        ...
        void AttachSchemaLookup(Node& node, const std::string& struct_name, int schema_index,
                                VTX::FieldType schema_type, VTX::FieldContainerType schema_container_type,
        ...
        const VTX::PropertyAddress* FindSchemaAddress(const VTX::StructSchemaCache* struct_cache,
                                                      VTX::FieldType expected_type,
                                                      VTX::FieldContainerType container_type, size_t index) {
            if (!struct_cache) {
                return nullptr;
            }
        ...
        void AttachSchemaPropertyLabel(Node& node, const std::string& parent_struct_name,
                                       const std::string& property_name, size_t index) {
        ...
        void AttachSchemaStructLabel(Node& node, const std::string& child_struct_name, const std::string& label) {
        ...
        bool HasFieldValue(const VTX::PropertyContainer& pc, const VTX::PropertyAddress& address) {
            ...
            const auto has_subarray_slot = [index = address.index](const auto& flat_array) {
                return index >= 0 && static_cast<size_t>(index) < flat_array.SubArrayCount();
            };

            if (address.container_type == VTX::FieldContainerType::Array) {
                switch (address.type_id) {
                case VTX::FieldType::Int8:
                    return has_subarray_slot(pc.byte_array_properties) || has_subarray_slot(pc.int32_arrays);
                case VTX::FieldType::Int32:
                case VTX::FieldType::Enum:
                    return has_subarray_slot(pc.int32_arrays);
                case VTX::FieldType::Int64:
                    return has_subarray_slot(pc.int64_arrays);
                case VTX::FieldType::Float:
                    return has_subarray_slot(pc.float_arrays);
                case VTX::FieldType::Double:
                    return has_subarray_slot(pc.double_arrays);
                case VTX::FieldType::Bool:
                    return has_subarray_slot(pc.bool_arrays);
                case VTX::FieldType::String:
                    return has_subarray_slot(pc.string_arrays);
                case VTX::FieldType::Vector:
                    return has_subarray_slot(pc.vector_arrays);
                case VTX::FieldType::Quat:
                    return has_subarray_slot(pc.quat_arrays);
                case VTX::FieldType::Transform:
                    return has_subarray_slot(pc.transform_arrays);
                case VTX::FieldType::FloatRange:
                    return has_subarray_slot(pc.range_arrays);
                case VTX::FieldType::Struct:
                    return has_subarray_slot(pc.any_struct_arrays);
            ...
            case VTX::FieldType::Int32:
            case VTX::FieldType::Enum:
                return has_index(pc.int32_properties.size());
            case VTX::FieldType::Int64:
                return has_index(pc.int64_properties.size());
            case VTX::FieldType::Float:
                return has_index(pc.float_properties.size());
            case VTX::FieldType::Double:
                return has_index(pc.double_properties.size());
            case VTX::FieldType::Bool:
                return has_index(pc.bool_properties.size());
            case VTX::FieldType::String:
                return has_index(pc.string_properties.size());
            case VTX::FieldType::Vector:
                return has_index(pc.vector_properties.size());
            case VTX::FieldType::Quat:
                return has_index(pc.quat_properties.size());
            case VTX::FieldType::Transform:
                return has_index(pc.transform_properties.size());
            case VTX::FieldType::FloatRange:
                return has_index(pc.range_properties.size());
            case VTX::FieldType::Struct:
        ...
        std::vector<std::string> BuildMissingFieldNames(const VTX::PropertyContainer& pc,
        ...
        void AppendPrimitiveNodes(std::vector<Node>& parent_children, const std::string& id_prefix,
                                  const char* group_title, const std::vector<T>& values, VTX::FieldType type_id,
                                  const VTX::StructSchemaCache* struct_cache, const std::string& struct_name,
                                  const char* label_prefix, DisplayFormatFn&& display_format_fn,
                                  RawFormatFn&& raw_format_fn) {
            if (values.empty()) {
                return;
            }

            Node group = MakeGroupNode(id_prefix + "/" + group_title,
            ...
            for (size_t i = 0; i < values.size(); ++i) {
                ...
                group.children.push_back(MakePropertyNode(
                    group.id + "/" + std::to_string(i), label, display_format_fn(values[i]), raw_format_fn(values[i]),
                    struct_name, static_cast<int>(i), type_id, VTX::FieldContainerType::None));
            }
        ...
        void AppendFlatArrayGroup(std::vector<Node>& parent_children, const std::string& id_prefix,
                                  const char* group_title, const FlatArrayT& flat_array, VTX::FieldType type_id,
                                  const VTX::StructSchemaCache* struct_cache, const std::string& struct_name,
                                  DisplayFormatFn&& display_format_fn, RawFormatFn&& raw_format_fn) {
            if (flat_array.SubArrayCount() == 0) {
                return;
            }

            Node arrays_group = MakeGroupNode(id_prefix + "/" + group_title,
                                              std::string(group_title) + " (" +
                                                  std::to_string(flat_array.SubArrayCount()) + " arrays)");
            ...
            for (size_t i = 0; i < flat_array.SubArrayCount(); ++i) {
                ...
                Node array_node =
                    MakeGroupNode(arrays_group.id + "/" + std::to_string(i),
                ...
                const std::string array_schema_name = EntityInspectorViewService::ResolveSchemaName(
                    struct_cache, type_id, VTX::FieldContainerType::Array, i);
                AttachSchemaLookup(array_node, struct_name, static_cast<int>(i), type_id,
                                   VTX::FieldContainerType::Array, true);
                ...
                for (size_t j = 0; j < sub_array.size(); ++j) {
                    ...
                    Node item_node =
                        MakePropertyNode(array_node.id + "/" + std::to_string(j), label,
                                         display_format_fn(sub_array[j]), raw_format_fn(sub_array[j]), struct_name,
                                         static_cast<int>(i), type_id, VTX::FieldContainerType::Array);
        ...
        void AppendMapEntries(Node& parent_node, const VTX::MapContainer& map_cont,
            ...
            for (size_t j = 0; j < map_cont.keys.size(); ++j) {
                Node key_node = MakeGroupNode(parent_node.id + "/" + std::to_string(j), "Key: " + map_cont.keys[j]);
                if (j < map_cont.values.size()) {
                    ...
                    if (!nested_struct_name.empty()) {
                        AttachSchemaStructLabel(key_node, nested_struct_name,
        ...
        void BuildPropertyNodesRecursive(const VTX::PropertyContainer& pc,
            ...
            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Booleans", pc.bool_properties, VTX::FieldType::Bool, struct_cache, struct_name,
                "Bool", [](bool value) { return value ? "True" : "False"; },
                [](bool value) { return value ? "True" : "False"; });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Int32", pc.int32_properties, VTX::FieldType::Int32, struct_cache, struct_name,
                "Int32", [](int32_t value) { return std::to_string(value); },
                [](int32_t value) { return std::to_string(value); });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Int64", pc.int64_properties, VTX::FieldType::Int64, struct_cache, struct_name,
                "Int64", [](int64_t value) { return std::to_string(value); },
                [](int64_t value) { return std::to_string(value); });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Floats", pc.float_properties, VTX::FieldType::Float, struct_cache, struct_name,
                "Float", [](float value) { return EntityInspectorViewService::FormatFloat(value); },
                [](float value) { return EntityInspectorViewService::FormatRawFloat(value); });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Doubles", pc.double_properties, VTX::FieldType::Double, struct_cache,
                struct_name, "Double", [](double value) { return EntityInspectorViewService::FormatDouble(value); },
                [](double value) { return EntityInspectorViewService::FormatRawDouble(value); });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Strings", pc.string_properties, VTX::FieldType::String, struct_cache,
                struct_name, "String", [](const std::string& value) { return value; },
                [](const std::string& value) { return value; });

            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Vectors", pc.vector_properties, VTX::FieldType::Vector, struct_cache,
                struct_name, "Vector",
            ...
            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Quaternions", pc.quat_properties, VTX::FieldType::Quat, struct_cache,
                struct_name, "Quat",
            ...
            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Transforms", pc.transform_properties, VTX::FieldType::Transform, struct_cache,
            ...
            AppendPrimitiveNodes(
                out_nodes, id_prefix, "Ranges", pc.range_properties, VTX::FieldType::FloatRange, struct_cache,
                struct_name, "Range",
            ...
            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Byte Arrays", pc.byte_array_properties, VTX::FieldType::Int8, struct_cache,
                struct_name,
                [](uint8_t value) {
                    char val[32];
            ...
            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Bool Arrays", pc.bool_arrays, VTX::FieldType::Bool, struct_cache, struct_name,
                [](uint8_t value) { return value != 0 ? std::string("True") : std::string("False"); },
                [](uint8_t value) { return value != 0 ? std::string("1") : std::string("0"); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Int32 Arrays", pc.int32_arrays, VTX::FieldType::Int32, struct_cache, struct_name,
                [](int32_t value) { return std::to_string(value); },
                [](int32_t value) { return std::to_string(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Int64 Arrays", pc.int64_arrays, VTX::FieldType::Int64, struct_cache, struct_name,
                [](int64_t value) { return std::to_string(value); },
                [](int64_t value) { return std::to_string(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Float Arrays", pc.float_arrays, VTX::FieldType::Float, struct_cache, struct_name,
                [](float value) { return EntityInspectorViewService::FormatFloat(value); },
                [](float value) { return EntityInspectorViewService::FormatRawFloat(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Double Arrays", pc.double_arrays, VTX::FieldType::Double, struct_cache,
                struct_name, [](double value) { return EntityInspectorViewService::FormatDouble(value); },
                [](double value) { return EntityInspectorViewService::FormatRawDouble(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "String Arrays", pc.string_arrays, VTX::FieldType::String, struct_cache,
                struct_name, [](const std::string& value) { return value; },
                [](const std::string& value) { return value; });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Vector Arrays", pc.vector_arrays, VTX::FieldType::Vector, struct_cache,
                struct_name, [](const VTX::Vector& value) { return EntityInspectorViewService::FormatVector(value); },
                [](const VTX::Vector& value) { return EntityInspectorViewService::FormatRawVector(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Quat Arrays", pc.quat_arrays, VTX::FieldType::Quat, struct_cache, struct_name,
                [](const VTX::Quat& value) { return EntityInspectorViewService::FormatQuat(value); },
                [](const VTX::Quat& value) { return EntityInspectorViewService::FormatRawQuat(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Transform Arrays", pc.transform_arrays, VTX::FieldType::Transform, struct_cache,
                struct_name,
                [](const VTX::Transform& value) { return EntityInspectorViewService::FormatTransform(value); },
                [](const VTX::Transform& value) { return EntityInspectorViewService::FormatRawTransform(value); });

            AppendFlatArrayGroup(
                out_nodes, id_prefix, "Range Arrays", pc.range_arrays, VTX::FieldType::FloatRange, struct_cache,
                struct_name,
                [](const VTX::FloatRange& value) { return EntityInspectorViewService::FormatRange(value); },
                [](const VTX::FloatRange& value) { return EntityInspectorViewService::FormatRawRange(value); });

            if (!pc.any_struct_properties.empty()) {
                Node group = MakeGroupNode(id_prefix + "/structs",
                ...
                for (size_t i = 0; i < pc.any_struct_properties.size(); ++i) {
                    ...
                    Node struct_node = MakeGroupNode(group.id + "/" + std::to_string(i),
                                                     "Struct [" + std::to_string(i) +
                                                         "] (Type ID: " + std::to_string(nested.entity_type_id) + ")");
                    const std::string struct_schema_name = EntityInspectorViewService::ResolveSchemaName(
                        struct_cache, VTX::FieldType::Struct, VTX::FieldContainerType::None, i);
                    AttachSchemaLookup(struct_node, struct_name, static_cast<int>(i), VTX::FieldType::Struct,
                                       VTX::FieldContainerType::None, true);
            ...
            if (pc.any_struct_arrays.SubArrayCount() > 0) {
                Node group = MakeGroupNode(id_prefix + "/struct_arrays",
                                           "Struct Arrays (" + std::to_string(pc.any_struct_arrays.SubArrayCount()) +
                                               " arrays)");

                for (size_t i = 0; i < pc.any_struct_arrays.SubArrayCount(); ++i) {
                    const auto sub_array = pc.any_struct_arrays.GetSubArray(i);
                    const std::string array_schema_name = EntityInspectorViewService::ResolveSchemaName(
                        struct_cache, VTX::FieldType::Struct, VTX::FieldContainerType::Array, i);
                    ...
                    AttachSchemaLookup(array_node, struct_name, static_cast<int>(i), VTX::FieldType::Struct,
                                       VTX::FieldContainerType::Array, true);
                    AttachSchemaPropertyLabel(array_node, struct_name, array_schema_name, i);

                    const std::string nested_schema_struct_name = [&]() -> std::string {
                        const auto* address =
                            FindSchemaAddress(struct_cache, VTX::FieldType::Struct, VTX::FieldContainerType::Array, i);
                    ...
                    for (size_t j = 0; j < sub_array.size(); ++j) {
                        ...
                        Node item_node =
                            MakeGroupNode(array_node.id + "/" + std::to_string(j), "Item [" + std::to_string(j) + "]");
                        ...
                        if (!nested_struct_name.empty()) {
                            AttachSchemaStructLabel(item_node, nested_struct_name,
            ...
            if (!pc.map_properties.empty()) {
                Node group =
                    MakeGroupNode(id_prefix + "/maps", "Maps (" + std::to_string(pc.map_properties.size()) + ")");

                for (size_t i = 0; i < pc.map_properties.size(); ++i) {
                    const auto& map_cont = pc.map_properties[i];
                    Node map_node = MakeGroupNode(group.id + "/" + std::to_string(i),
                                                  "Map [" + std::to_string(i) + "] (" +
                                                      std::to_string(map_cont.keys.size()) + " pairs)");
                    const std::string map_schema_name = EntityInspectorViewService::ResolveSchemaName(
                        struct_cache, VTX::FieldType::Struct, VTX::FieldContainerType::Map, i);
            ...
            if (pc.map_arrays.SubArrayCount() > 0) {
                Node group = MakeGroupNode(id_prefix + "/map_arrays",
                ...
                for (size_t i = 0; i < pc.map_arrays.SubArrayCount(); ++i) {
                    ...
                    Node array_node = MakeGroupNode(group.id + "/" + std::to_string(i),
                                                    "Array [" + std::to_string(i) + "] (" +
                                                        std::to_string(sub_array.size()) + " maps)");
                    ...
                    for (size_t j = 0; j < sub_array.size(); ++j) {
                        ...
                        Node map_node = MakeGroupNode(array_node.id + "/" + std::to_string(j),
                                                      "Map [" + std::to_string(j) + "] (" +
                                                          std::to_string(map_cont.keys.size()) + " pairs)");
    ...
    FrameResolveResult EntityInspectorViewService::ResolveFrameToDraw(VTX::IVtxReaderFacade& reader,
                                                                      const FrameResolveContext& context) {
    ...
    std::vector<EntityBucketView>
    EntityInspectorViewService::BuildEntityBuckets(const VTX::Frame& frame, const EntityInspectorState& state,
                                                   const VTX::PropertyAddressCache* global_cache) {
        ...
        for (const auto& bucket : buckets) {
            ...
            bucket_vm.label =
                "Bucket " + std::to_string(current_bucket_idx) + " (" + std::to_string(bucket.entities.size()) + ")";
            ...
            for (size_t i = 0; i < bucket.entities.size(); ++i) {
                const std::string entity_id =
                    (i < bucket.unique_ids.size()) ? bucket.unique_ids[i] : "UnknownEntity_" + std::to_string(i);
                const int entity_type_id = bucket.entities[i].entity_type_id;
                const std::string entity_type_name =
                    ResolveStructName(ResolveStructCache(global_cache, entity_type_id));
    ...
    EntityInspectorViewService::BuildPropertyTree(const VTX::PropertyContainer& pc,
                                                  const VTX::PropertyAddressCache* global_cache) {
    ...
    EntityInspectorViewService::BuildActivateCommand(const EntityPropertyNode& node,
                                                     const VTX::PropertyAddressCache* global_cache) {
    ...
    EntityPropertyCommand
    EntityInspectorViewService::BuildSchemaFocusCommand(const EntityPropertyNode& node,
                                                        const VTX::PropertyAddressCache* global_cache) {
        EntityPropertyCommand command;
    ...
    EntityInspectorScreenResult EntityInspectorViewService::BuildScreen(VTX::IVtxReaderFacade* reader,
                                                                        bool has_loaded_replay,
                                                                        bool is_scrubbing_timeline, int current_frame,
        ...
        const auto frame_result =
            ResolveFrameToDraw(*reader, FrameResolveContext {
        ...
        result.view_model.status_tone =
            frame_result.is_loading ? (is_scrubbing_timeline ? EntityStatusTone::Warning : EntityStatusTone::Error)
                                    : EntityStatusTone::Normal;
        if (frame_result.showing_stale_frame) {
            result.view_model.stale_frame_message =
        ...
        result.view_model.status_tone =
            frame_result.is_loading ? (is_scrubbing_timeline ? EntityStatusTone::Warning : EntityStatusTone::Error)
                                    : EntityStatusTone::Normal;
        result.view_model.stale_frame_message =
            frame_result.showing_stale_frame
                ? "(Showing stale frame " + std::to_string(frame_result.stale_frame_index) + ")"
                : std::string {};
        return result;
    }

    EntityInspectorState EntityInspectorViewService::SelectEntity(const EntityInspectorState& state, int bucket_index,
                                                                  const std::string& entity_id) {
    ...
    EntityInspectorEffect
    EntityInspectorViewService::BuildPropertyActivateEffect(const EntityPropertyNode& node,
                                                            const VTX::PropertyAddressCache* global_cache) {
    ...
    EntityInspectorEffect
    EntityInspectorViewService::BuildPropertyContextEffect(const EntityPropertyNode& node,
                                                           const VTX::PropertyAddressCache* global_cache) {
    ...
    const VTX::PropertyContainer* EntityInspectorViewService::FindSelectedEntity(const VTX::Frame& frame,
                                                                                 const EntityInspectorState& state) {
    ...
    EntityInspectorViewService::BuildFrameViewModel(const VTX::Frame& frame, const EntityInspectorState& state,
                                                    const VTX::PropertyAddressCache* global_cache) {
        ...
        view_model.header.entity_label =
            "Entity: " + state.selected_entity_id + " (Bucket " + std::to_string(state.selected_bucket_index) + ")";
    ...
    const VTX::StructSchemaCache*
    EntityInspectorViewService::ResolveStructCache(const VTX::PropertyAddressCache* global_cache,
    ...
    std::string EntityInspectorViewService::ResolveSchemaName(const VTX::StructSchemaCache* struct_cache,
                                                              VTX::FieldType expected_type,
                                                              VTX::FieldContainerType container_type, size_t index) {
        if (!struct_cache) {
            return "";
    ...
    std::string EntityInspectorViewService::ResolveSchemaName(const VTX::PropertyAddressCache* global_cache,
        ...
        return ResolveSchemaName(struct_cache, node.schema_type, node.schema_container_type,
    ...
    std::string EntityInspectorViewService::FormatTransform(const VTX::Transform& t) {
        char val[320];
        std::snprintf(val, sizeof(val), "Loc(%.1f, %.1f, %.1f) Rot(%.2f, %.2f, %.2f, %.2f) Scale(%.2f, %.2f, %.2f)",
                      t.translation.x, t.translation.y, t.translation.z, t.rotation.x, t.rotation.y, t.rotation.z,
                      t.rotation.w, t.scale.x, t.scale.y, t.scale.z);
        return val;
    }
    ...
    std::string EntityInspectorViewService::FormatRawVector(const VTX::Vector& v) {
        char val[160];
        std::snprintf(val, sizeof(val), "X: %.*g Y: %.*g Z: %.*g", std::numeric_limits<float>::max_digits10, v.x,
                      std::numeric_limits<float>::max_digits10, v.y, std::numeric_limits<float>::max_digits10, v.z);
        return val;
    }

    std::string EntityInspectorViewService::FormatRawQuat(const VTX::Quat& q) {
        char val[192];
        std::snprintf(val, sizeof(val), "X: %.*g Y: %.*g Z: %.*g W: %.*g", std::numeric_limits<float>::max_digits10,
                      q.x, std::numeric_limits<float>::max_digits10, q.y, std::numeric_limits<float>::max_digits10, q.z,
                      std::numeric_limits<float>::max_digits10, q.w);
        return val;
    }
    ...
    std::string EntityInspectorViewService::FormatRawTransform(const VTX::Transform& t) {
        char val[384];
        std::snprintf(
            val, sizeof(val), "Loc(%.*g, %.*g, %.*g) Rot(%.*g, %.*g, %.*g, %.*g) Scale(%.*g, %.*g, %.*g)",
            std::numeric_limits<float>::max_digits10, t.translation.x, std::numeric_limits<float>::max_digits10,
            t.translation.y, std::numeric_limits<float>::max_digits10, t.translation.z,
            std::numeric_limits<float>::max_digits10, t.rotation.x, std::numeric_limits<float>::max_digits10,
            t.rotation.y, std::numeric_limits<float>::max_digits10, t.rotation.z,
            std::numeric_limits<float>::max_digits10, t.rotation.w, std::numeric_limits<float>::max_digits10, t.scale.x,
            std::numeric_limits<float>::max_digits10, t.scale.y, std::numeric_limits<float>::max_digits10, t.scale.z);
        return val;
    }

    std::string EntityInspectorViewService::FormatRawRange(const VTX::FloatRange& r) {
        char val[192];
        std::snprintf(val, sizeof(val), "Min: %.*g Max: %.*g ValNorm: %.*g", std::numeric_limits<float>::max_digits10,
                      r.min, std::numeric_limits<float>::max_digits10, r.max, std::numeric_limits<float>::max_digits10,
                      r.value_normalized);
        return val;
    }
