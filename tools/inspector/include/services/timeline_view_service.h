...
public:
    ...
    static TimelineClockSpan BuildTimelineClockSpan(
        int current_frame,
        int total_frames,
    ...
    static TimelineVisibleRange ComputeVisibleRange(float scroll_x, float view_width, int total_frames, const TimelineBarState& bar_state);
    static float ComputeZoomedScroll(float wheel, float mouse_x_in_child, float scroll_x, TimelineBarState& bar_state);
    static int ResolveHoveredFrame(float mouse_x, float mouse_y, float origin_x, float origin_y, float timeline_height, const TimelineVisibleRange& range);
};
