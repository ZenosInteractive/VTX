...
namespace VtxServices {
    ...
    public:
        ...
        static std::vector<EntityBucketView> BuildEntityBuckets(const VTX::Frame& frame,
                                                                const EntityInspectorState& state,
                                                                const VTX::PropertyAddressCache* global_cache);
        static EntityPropertiesViewModel BuildPropertyTree(const VTX::PropertyContainer& pc,
                                                           const VTX::PropertyAddressCache* global_cache);
        static EntityInspectorScreenResult BuildScreen(VTX::IVtxReaderFacade* reader, bool has_loaded_replay,
                                                       bool is_scrubbing_timeline, int current_frame,
                                                       const EntityInspectorState& state);
        static EntityInspectorState SelectEntity(const EntityInspectorState& state, int bucket_index,
                                                 const std::string& entity_id);
        static EntityInspectorEffect BuildPropertyActivateEffect(const EntityPropertyNode& node,
                                                                 const VTX::PropertyAddressCache* global_cache);
        static EntityInspectorEffect BuildPropertyContextEffect(const EntityPropertyNode& node,
                                                                const VTX::PropertyAddressCache* global_cache);
        static const VTX::PropertyContainer* FindSelectedEntity(const VTX::Frame& frame,
                                                                const EntityInspectorState& state);
        static EntityInspectorViewModel BuildFrameViewModel(const VTX::Frame& frame, const EntityInspectorState& state,
                                                            const VTX::PropertyAddressCache* global_cache);

        static const VTX::StructSchemaCache* ResolveStructCache(const VTX::PropertyAddressCache* global_cache,
                                                                int32_t entity_type_id);
        static const VTX::StructSchemaCache* ResolveStructCache(const VTX::PropertyAddressCache* global_cache,
                                                                const std::string& struct_name);
        static std::string ResolveSchemaName(const VTX::StructSchemaCache* struct_cache, VTX::FieldType expected_type,
                                             VTX::FieldContainerType container_type, size_t index);
        static std::string ResolveSchemaName(const VTX::PropertyAddressCache* global_cache,
    ...
    private:
        static EntityPropertyCommand BuildActivateCommand(const EntityPropertyNode& node,
                                                          const VTX::PropertyAddressCache* global_cache);
        static EntityPropertyCommand BuildSchemaFocusCommand(const EntityPropertyNode& node,
                                                             const VTX::PropertyAddressCache* global_cache);
    };
