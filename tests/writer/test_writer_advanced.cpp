...
TEST_P(WriterAdvancedTest, ThrowsWhenParentDirectoryDoesNotExist) {
    ...
    const std::filesystem::path missing_dir =
        std::filesystem::path(VtxTest::OutputPath(std::string("writer_missing_dir_") + FormatName(GetParam()))) /
        "nested";
...
TEST_P(WriterAdvancedTest, CreatesParentDirectoryWhenMissing) {
    ...
    const std::filesystem::path root =
        std::filesystem::path(VtxTest::OutputPath(std::string("writer_autocreate_") + FormatName(GetParam())));
