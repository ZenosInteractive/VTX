...
TEST(TimeUtils, ConvertToUeTicksSupportsNumericFormats)
{
    ...
    EXPECT_EQ(ConvertToUeTicks(TimeFormat::UeUTC, 123456789LL), 123456789LL);
    EXPECT_EQ(ConvertToUeTicks(TimeFormat::UnixUTC, 0LL), TICKS_AT_UNIX_EPOCH);
    EXPECT_EQ(ConvertToUeTicks(TimeFormat::UnixMsUTC, 1LL),
              TICKS_AT_UNIX_EPOCH + TICKS_PER_MILLISECOND);
    EXPECT_EQ(ConvertToUeTicks(TimeFormat::Seconds, 1.5f), 15'000'000LL);

    EXPECT_EQ(ConvertToUeTicks(TimeFormat::Seconds, 7LL), 0LL);
}
...
TEST(TimeUtils, ConvertToUeTicksParsesIso8601AndRejectsInvalidStrings)
{
    ...
    EXPECT_EQ(ConvertToUeTicks(TimeFormat::ISO8601, std::string("1970-01-01T00:00:00")),
              TICKS_AT_UNIX_EPOCH);
...
TEST(TimeUtils, DurationHelpersFormatPredictably)
{
    ...
    const int64 ticks =
        (3LL * 3600 * TICKS_PER_SECOND) +
        (2LL * 60 * TICKS_PER_SECOND) +
        (5LL * TICKS_PER_SECOND) +
...
TEST(TimeUtils, CalculateContainerHashIncludesFlatArrayOffsets)
{
    ...
    EXPECT_NE(VTX::Helpers::CalculateContainerHash(a),
              VTX::Helpers::CalculateContainerHash(b));
}
