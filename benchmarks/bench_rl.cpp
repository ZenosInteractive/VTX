...
std::string RlReplayPath(const char* backend_filename) {
    return (std::filesystem::path(VTX_BENCH_FIXTURES_DIR).parent_path().parent_path()
            / "samples" / "content" / "reader" / "rl" / backend_filename)
        .string();
}
...
BENCHMARK(BM_RL_ReaderSequentialScan_FBS)
...
BENCHMARK(BM_RL_ReaderSequentialScan_Proto)
    ->Unit(benchmark::kMillisecond)
    BENCH_HEAVY_FIXTURE_SUFFIX;
...
BENCHMARK(BM_RL_AccessorSequentialScan_FBS)
    ->Unit(benchmark::kMillisecond)
    BENCH_HEAVY_FIXTURE_SUFFIX;
